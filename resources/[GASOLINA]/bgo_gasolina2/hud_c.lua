...
function dxDrawSpeedo ()
		local car = getPedOccupiedVehicle ( localPlayer )
			if ( isElement ( car ) ) then
				...
				--if frontLeft == 1 or rearLeft == 1 or frontRight == 1 or rearRight == 1 and getVehicleEngineState ( car ) then 
					--exports.bgo_hud:dm("SEU VEICULO ESTÁ COM O PNEU FURADO! CHAME O MECANICO!", 255, 125, 0)
					--setVehicleEngineState ( car, false )
				...
				local health2 = math.floor ( health )
				if ( tonumber(health2) <= 340 and getVehicleEngineState ( car ) ) then
					setVehicleEngineState ( car, false )
					exports.bgo_hud:dm("Seu veiculo esta com motor quebrado", 255, 125, 0)
				end
...
--[[
				...
				if ( tonumber(fuel2) <= 0 and getVehicleEngineState ( car ) ) then
					setVehicleEngineState ( car, false )
					exports.bgo_hud:dm("Seu veiculo esta sem gasolina!", 255, 125, 0)
				end
		end
end
addEventHandler ( "onClientPreRender", root, dxDrawSpeedo )