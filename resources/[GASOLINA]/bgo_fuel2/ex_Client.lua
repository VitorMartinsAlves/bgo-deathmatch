...
local ped_info = {
	...
	{180, 1928.2584228516,-1770.5417480469,13.655365943909, 7.1054158210754, 0, 0, "POSTO DE BAIXO - Pagamento Gasolina!", 0, 0}, -- északi 
	{180, 1002.4893188477, -923.24926757813, 42.328125, -95, 0, 0, "Jabaquara - Pagamento Gasolina!", 0, 0}, -- Déli 
	{180, 1233.6147460938,-1465.1674804688,13.548749923706, 1.0061483383179, 0, 0, "CENTRAL POSTO - Pagamento Gasolina!", 0, 0}, 
	{180, 1462.6047363281,-1820.7161865234,13.546875, 3.4559667110443, 0, 0, "Estacionamento DP - Pagamento Gasolina!", 0, 0}, 
	...
	{284, 285.61633300781,1940.2979736328,17.640625, 185.50646972656, 0, 0, "Força Aérea - Pagamento Gasolina!", 0, 0}, 
    ...
    {16, 1269.7591552734,-1696.5093994141,34.8046875, 87.728485107422, 0, 0, "HELIPONTO LS - Pagamento Gasolina!", 0, 0},

    {16, 1345.2565917969,1756.2579345703,10.8203125, 271.54125976563, 0, 0, "HELIPONTO LV - Pagamento Gasolina!", 0, 0},

    {16, -1193.9271240234,18.289937973022,14.1484375, 315.21838378906, 0, 0, "HELIPONTO SF - Pagamento Gasolina!", 0, 0},
...
addEventHandler("onClientResourceStart", resourceRoot, function ()
	for index, value in ipairs (ped_info) do 
		...
		--setElementDimension(fuelPed[index], 9)

		

		--local myBlip = createBlip( value[2], value[3], value[4], 14, 3, 255, 0, 0 )

		local myBlip = createBlipAttachedTo ( fuelPed[index], 14 )
...
addEventHandler("onClientRender", root, function ()
	if  clickElement[1]  then 
		...
		if getDistanceBetweenPoints3D(playerX, playerY, playerZ, lineX, lineY, lineZ) < 5 then 
			...
			exports.bgo_chat:sendLocalMeMessage(localPlayer, "Colocou a mangueira de volta no suporte.")
			setElementData(localPlayer, "fuel_gun", false)
		...
		if getElementData(localPlayer, "fuel_gun") or false then 
			...
			for k, v in ipairs(getElementsByType("vehicle")) do
				...
				if getElementDimension(v) == 0 then
					VehicleID = getElementData(v, "veh:id")
					VehicleElement = v
					...
					if getDistanceBetweenPoints3D(x,y,z, px, py, pz) < 1 then 
						...
						if getVehicleEngineState(VehicleElement) then
							...
							dxDrawText("Desligue o motor para reabastecer." ,monitorSize[1]-250-10+250/2,monitorSize[2]/2-128+135+50/2, monitorSize[1]-250-10+250/2 , monitorSize[2]/2-128+135+50/2,  tocolor(255, 255, 255, 255), 1, font2, "center", "center", false, false, false, true)
						end
			...
			if VehicleElement and not getVehicleEngineState(VehicleElement) then 
				if clickElement[2] and getKeyState("mouse1") then 
					...
					triggerServerEvent("bgo:AttLitro", localPlayer, localPlayer, math.floor(gallon))
					
					
					if getElementData(localPlayer, "char:money") >= math.floor(fuel) then
					else
					...
					if math.floor(getElementData(VehicleElement, "veh:fuel")) >= 100 then 
						...
						if gallon >= (100 - getElementData(VehicleElement, "veh:fuel")) then 
...
function gasolina()
							...
							if clickElement[3] then 
								triggerServerEvent("playerAnimationToServer", localPlayer, localPlayer, nil, nil)
								exports.bgo_chat:sendLocalMeMessage(localPlayer, "Colocou a mangueira de volta no suporte.")
								triggerEvent("bgo>info", localPlayer,"Posto BGO", "Esta bomba está sem gasolina!", "aviso")
							end
							clickElement[1]  = false
							clickElement[2] = false 
							clickElement[3] = false 
...
addEvent("cancelargasolinabomba", true)
addEventHandler("cancelargasolinabomba", root, gasolina)
...
addEventHandler("onClientClick", root, function (button, state, absoluteX, absoluteY, worldX, worldY, worldZ, clickedElement)
	if button == "right" and state == "down" and not isPedInVehicle(localPlayer) then 
		...
		element = clickedElement	
			...
			if getElementData(clickedElement, 'Dono >> bomba') == getElementData(localPlayer, "char:id") then
			...
			if getElementData(localPlayer, "fuel_gun") or false then
				...
				setElementData(clickedElement, "fuel:dono", localPlayer)
				setElementData(localPlayer, "fuel_gun", true)
				exports.bgo_chat:sendLocalMeMessage(localPlayer, "Pegou a mangueira para abastecer seu veiculo")
			end


			else
			
			if  getElementData(localPlayer, "donobomba") == false then
			
			if  getElementData(clickedElement, 'Dono >> bomba') == false then 
			...
			if getElementData(localPlayer, "fuel_gun") or false then
				...
				setElementData(localPlayer, "fuel_gun", true)
				exports.bgo_chat:sendLocalMeMessage(localPlayer, "Pegou a mangueira para abastecer seu veiculo")
				triggerServerEvent("bgoMTA->#DonoBomba", localPlayer, localPlayer)
			end
			
			else
				outputChatBox("[bgoMTA ~ Miner]: #ffffffEsta bomba ja está em uso, utilize outra!", 124, 197, 118, true)
				end
			else
				outputChatBox("[bgoMTA ~ Miner]: #ffffffVocê ja está utilizando uma bomba seja humilde!", 124, 197, 118, true)
		...
		elseif clickedElement and getElementData(clickedElement, "fuelPed:ID") or 0 > 0 and not clickElement[4] then 
				...
				if getDistanceBetweenPoints3D(x, y, z, elementX, elementY, elementZ) >= 3 then 
					outputChatBox('#D24D57[Bgo~Gasolina] #ffffffVocê não pode falar com o vendedor nessa distância!', 255, 255, 255, true)
					return
				end
	...
	elseif button == "left" and state == "down" then 
		if clickElement[4] then 
			if dobozbaVan(monitorSize[1]/2-300/2+300/2-90 ,monitorSize[2]/2-500/2+330, 180, 40, absoluteX, absoluteY) then 
				if VehicleElement and gallon > 0 and getElementData(localPlayer, "char:money") >= math.floor(fuel) then
					if getElementData(VehicleElement, "veh:fuel") < 101 and (getElementData(VehicleElement, "veh:fuel") + gallon) < 101  then 
						setElementData(localPlayer, "char:money", getElementData(localPlayer, "char:money") - math.floor(fuel))
						setElementData(VehicleElement, "veh:fuel", getElementData(VehicleElement, "veh:fuel") + math.floor(gallon))
						triggerServerEvent("bgo:ComprarLitro", localPlayer, localPlayer)
						fuel = 0
						gallon = 0
						...
						clickElement[2] = false
						clickElement[1] = false
				if VehicleElement and getElementData(VehicleElement, "veh_fueling") then
				setElementData(VehicleElement, "veh_fueling", false)
				end
				...
				setElementData(VehicleElement, "veh_fueling", false)
						...
						VehicleElement = false
						triggerServerEvent("bgo:cancelarLitro", localPlayer, localPlayer, gallon)
					end
				end
...
setElementData(localPlayer, "donobomba", false)
...
addEventHandler("onClientRender", root, function()
	for k,v in ipairs(getElementsByType("object", getResourceRootElement(getThisResource()), true)) do
		if isElement(v) and tonumber(getElementData(v, 'dbid') or 0) > 0 then 
			local x, y ,z = getElementPosition(v)
			...
			if wx and wy then
				...
				if getDistanceBetweenPoints3D(playerx, playery, playerz, x, y ,z) <= 6 then
					local stone_HP = (getElementData(v, 'fuel >> Health') or 0)
					local mensagem = getElementData(v, 'fuel >> mensagem')
					local quant = getElementData(v, 'fuel >> quantidade')
					
					if tonumber(quant) > 1 then --and stone_HP > 0 then
					--[[
						...
						dxDrawRectangle(wx-200/2,wy,200,30,tocolor(0,0,0,170))
						--dxDrawRectangle(wx-200/2+2,wy+2, (stone_HP/1000)*(200 - 4) ,26,tocolor(124, 197, 118, 190))
						dxDrawText(''..format(math.floor(quant)) .. ' Litros',  wx-200/2 + 200/2+1, wy+30/2, wx-200/2 + 200/2+1, wy+30/2, tocolor(0, 0, 0,255),1, "default", "center","center",false,false,false,true,false)
						dxDrawText(''..format(math.floor(quant)) .. ' Litros', wx-200/2 + 200/2, wy+30/2, wx-200/2 + 200/2, wy+30/2, tocolor(255, 255, 255,255),1, "default", "center","center",false,false,false,true,false)
			else
					
						dxDrawRectangle(wx-200/2,wy,200,30,tocolor(0,0,0,170))
						dxDrawText('Esta bomba está sem gasolina!',  wx-200/2 + 200/2+1, wy+30/2, wx-200/2 + 200/2+1, wy+30/2, tocolor(0, 0, 0,255),1, "default", "center","center",false,false,false,true,false)
						dxDrawText('Esta bomba está sem gasolina!', wx-200/2 + 200/2, wy+30/2, wx-200/2 + 200/2, wy+30/2, tocolor(255, 255, 255,255),1, "default", "center","center",false,false,false,true,false)
					end
				end
			end
		end
