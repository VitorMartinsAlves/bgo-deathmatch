...
function loadGPS()
	...
	--dxDrawRectangle (windowStart[1], windowStart[2], windowSize[1], windowSize[2], tocolor ( 0, 191, 255, 155 ) )
...
--[[
	...
	for k, v in ipairs(gpsPontok) do
		...
		dxDrawImage( 0, 0, scx, scy, radarTab.quad, 0, 0, 0, tocolor(124, 197, 200,255) )	
...
--[[
	...
	dxDrawRectangle(0, 0, windowSize[1], windowSize[2], tocolor(65,131,215, 215))

	dxDrawImageSection( 0, 0, windowSize[1], windowSize[2],
                          0, 0, windowSize[1], windowSize[2], radarTab.renderTarget, 0, 0, 0, tocolor(255, 255, 255, 215) )
	dxSetRenderTarget()
	...
	for k, v in ipairs(getElementsByType("blip")) do
		--if getElementData(localPlayer, "acc:admin") == 10 then
		...
		if bliPos[1] and bliPos[2] then
			...
			if getBlipIcon(v) == 1 then
				...
				dxDrawImage( 0, 0, scx, scy, radarTab.blip, 0, 0, 0, color)

			else

			local actualDist = getDistanceBetweenPoints2D(plaPos[1], plaPos[2], bliPos[1], bliPos[2])
			if actualDist <= getBlipVisibleDistance(v) and getElementDimension(v) == plDimension and getElementInterior(v) == plInterior then  --530
...
function createTileTextures()
	...
	for x=0, 5, 1 do
		...
		for y=0, 5, 1 do
			...
			radarTab.tiles[x][y].texture = dxCreateTexture("tex/radar/tile_"..y.."_"..x..".png")
