...
local function doPoolShot(x,y,x2,y2)
	...
	if not hit or not hitElement then
		triggerServerEvent("broadcastCaptionedEvent", localPlayer, getPlayerName(localPlayer) .. " você não bateu em nenhuma bola.", 5, 5, true)
	end
	local model=getElementModel(hitElement)
	if (ballNames[model]) then
		triggerServerEvent("broadcastCaptionedEvent", localPlayer, getPlayerName(localPlayer) .. " bateu na "..ballNames[model]..".", 5, 5, true)
	end
