...
local tables={ 	-- define table positions here
  -- x,y,z, dimension, interior
	{ 1580.5994873047,-1253.0184326172,277.87957763672, 0,0}, -- two tables at grove street
	--{ 998.73791503906,-1959.9404296875,19.855363845825, 0,0},
...
addEventHandler("onColShapeHit", resourceRoot, function(el,md)
	...
	if st and st=="pot" then
		...
		if ballNames[model] and deleteBall(el, idx) then
			triggerEvent("broadcastCaptionedEvent", source, ballNames[model].." foi encaçapada.", 5, 5, true)
		end
		return
	end
...
--				outputDebugString(tostring((x-x2)/))
				...
				if (not tables[tableNumber].timer) then
					tables[tableNumber].timer=setTimer(billardProcess, 55, 0, tableNumber)
				end
				return
			end
	end
...
function billardProcess(tableNumber)
	if (getTickCount()-(tables[tableNumber].lastTick or 0)<15) then return end
...
addCommandHandler("redefinir",function(plr,cmd,tn)
	tn=tonumber(tn)

	if not tn then
		outputChatBox("Use: /redefinir <numero da mesa(1,2)>", plr)
		return
	end
