kukaterulet2 = createColSphere(965.7880859375, -2155.02734375, 13.0859375,25)
...
function hill_Enter ( thePlayer, matchingDimension )
	...
	local hours = time.hour
	 if hours < 20 then
		 exports.bgo_info:addNotification(thePlayer,"[BGO ERROR]: Horário disponivel do lixão: (20 Hrs ás 22 Hrs).","info")
		 return
	 end
	 if hours > 21 then
		 exports.bgo_info:addNotification(thePlayer,"[BGO ERROR]: Horário disponivel do lixão: (20 Hrs ás 22 Hrs).","info")
...
addEventHandler ( "onColShapeHit", kukaterulet2, hill_Enter )
...
function turkal3(player)
	...
	local hours = time.hour
	 if hours < 20 then
		 exports.bgo_info:addNotification(player,"[BGO ERROR]: Horário disponivel do lixão: (20 Hrs ás 22 Hrs).","info")
		 return
	 end
	 if hours > 21 then
		 exports.bgo_info:addNotification(player,"[BGO ERROR]: Horário disponivel do lixão: (20 Hrs ás 22 Hrs).","info")
	...
	if getElementData(player,"char:adminduty") == 0 then
		...
		if not isPedInVehicle(player) then 
			if isElementWithinColShape(player,kukaterulet2) then
...
--exports.bgo_chat:sendLocalMeAction(player," Encontrei um item: ".. exports["bgo_items"]:getItemName(randoms[randomcucc][1]))
					...
					elseif randoms[randomcucc][2] == "hp" then
						...
						outputChatBox("#c0c0c0[Lixão]:#FFFFFF Você furou a mão em algo enferrujado e perdeu #D75656-".. randoms[randomcucc][1] .." #ffffffhp-t.",player,255,0,0,true)
			...
			--	outputChatBox("#c0c0c0[Lixão]:#FFFFFF Nem vagy a szemét telepen!",player,255,255,255,true)
			--	exports.bgo_info:addNotification(player,"Nem vagy a szemét telepen.","error")
			end
		else
