...
maxCraftRecipe = 15 -- // max Craft receptek
...
function drawInventory()
    ...
    --dxDrawRectangle(panelX, panelY, width, height, tocolor(0, 0, 0, 150)) -- BackGround
	...
	dxDrawRectangle(panelX, panelY, width, height, tocolor(0, 0, 0, 150)) -- BackGround
    --dxDrawRectangle(panelX, panelY, width, 33, tocolor(0, 0, 0, 255))
	local nome = getItemName(UsarItem[3])
    dxDrawText(nome, panelX + width / 2, panelY + height - 1313 / 2, panelX + width / 2, panelY + height + 33 / 2, tocolor(0, 0, 0, 255), 1.5, 'pricedown', 'center', 'center', false, false, false, true)
    dxDrawText(nome, panelX + width / 2, panelY + height - 1323 / 2, panelX + width / 2, panelY + height + 33 / 2, tocolor(255, 255, 255, 255), 1.5, 'pricedown', 'center', 'center', false, false, false, true)

    for index, value in ipairs(showmenu) do
		dxDrawButton(value, panelX + width / 2 - 50, panelY + height - 223  / 2 - index * 40, sx*80, sy*20, tocolor(255, 255, 255, 100))
    end
	...
	dxDrawRectangle(panelX + width - 66, panelY + 8, 64, 19, tocolor(0, 0, 0, 150))
    if isElement(btn_edit) then
        ...
        if tonumber(countText) then
            ...
            selectedAmount = 0
            dxDrawText("Numero", panelX + width - 66 + 64 / 2, panelY - 21 + 19 / 2, panelX + width - 66 + 64 / 2, panelY - 21 + 19 / 2, tocolor(255, 255, 255, 255), 1, font1, "center", "center", false, false, false, true)
        end
    end
    ...
    for index, value in ipairs(inventoryElem) do
        if isMouseInPosition(panelX - 150 + width - index * 40, panelY - 15, 35, 35) and not confirmando  or value == activeInvState and not confirmando  then
            dxDrawImage(panelX - 150 + width - index * 40, panelY - 15, 35, 35, 'img/' .. value .. '.png', 0, 0, 0, tocolor(255, 255, 255, 255)) -- 40, 48
        else
            dxDrawImage(panelX - 150 + width - index * 40, panelY - 15, 35, 35, 'img/' .. value .. '.png', 0, 0, 0, tocolor(255, 255, 255, 180)) -- 40, 48
        end
    end
    ...
    if activeInvState ~= 'craft' then
        ...
        --dxDrawText("#FF8800Mochila", panelX + 10, panelY + 35 / 2, width, panelY + 35 / 2, tocolor(255, 255, 255, 255), 1, font, 'left', 'center', false, false, false, true)
		texto = "Mochila"
		elseif getElementType(inventoryElement) == "vehicle" then
		texto = "Porta-Malas"
        --dxDrawText("#FF8800Veículo", panelX + 10, panelY + 35 / 2, width, panelY + 35 / 2, tocolor(255, 255, 255, 255), 1, font, 'left', 'center', false, false, false, true)
        elseif getElementType(inventoryElement) == "object" then
		texto = "Cofre"
        --dxDrawText("#FF8800Baú", panelX + 10, panelY + 35 / 2, width, panelY + 35 / 2, tocolor(255, 255, 255, 255), 1, font, 'left', 'center', false, false, false, true)
        end
		
		dxDrawText(texto, panelX + width / 2, panelY + height + 93 / 2, panelX + width / 2, panelY + height + 33 / 2, tocolor(0, 0, 0, 255), sx*1, 'pricedown', 'center', 'center', false, false, false, true)
	...
	dxDrawRectangle(panelX, panelY + 30, width, height - 30, tocolor(0, 0, 0, 150))
        ...
        for index, value in ipairs(craftLists) do
            if (index > scroll and elem < maxCraftRecipe) then
                elem = elem + 1
                dxDrawRectangle(panelX + width - 215, panelY + 36 - 16 + elem * 21, itemSize, itemSize, tocolor(0, 0, 0, 255))
                ...
                if isMouseInPosition(panelX + width - 215, panelY + 36 - 16 + elem * 21, 200 - 10, 20) and not confirmando  then
                    local isFaction = ""
                    local isDimension = ""
                    local isTools = ""
...
function clickInventory(button, state, _, _, _, _, _, element)
    ...
    elseif button == 'left' and state == 'down' and showInventory and not confirmando then
        ...
        if getElementType(inventoryElement) == 'player' then
            for index, value in ipairs(inventoryElem) do
                if isMouseInPosition(panelX - 150 + width - index * 40, panelY - 15, 25, 25) and not startCraft then
	...
	elseif value == "Mostrar" then
        local startTime = getTickCount()
        setElementData(localPlayer, "char >> showedItem", { true, UsarItem[2], getItemName(UsarItem[2]), getItemDescription(UsarItem[2]), getTickCount() })
        outputChatBox("Mostrou o item: #F7CA18" .. getElementData(localPlayer, "char >> showedItem")[3], 255, 255, 255, true)
        triggerServerEvent("btcMTA->#setPlayerMe", localPlayer, localPlayer, "Está mostrando um item (" .. getItemName(getElementData(localPlayer, "char >> showedItem")[2]) .. ")")
        setTimer(function()
        setElementData(localPlayer, "char >> showedItem", { false, nil, nil, nil, nil })
