...
addEventHandler('btcMTA->#movedItemToElement', root, function(element, targetElement, slot, item, val, count)
	...
	if getElementData(element, "char.Cuffed") == 1 then 
	triggerClientEvent(element, "bgo>info", element,"Informação", "Você não pode passar item algemado!", "info")
	return 
	end
	
	if getElementData(targetElement, "char.Cuffed") == 1 then 
	triggerClientEvent(element, "bgo>info", element,"Informação", "Você não pode passar item para alguem algemado!", "info")
	return 
	end
    ...
    if giveItem(targetElement, item, val, count, 0, true) then
        ...
        setPlayerAnimation2(element, "DEALER", "DEALER_DEAL", 3000)
        setPlayerAnimation2(targetElement, "DEALER", "DEALER_DEAL", 3000)
...
function giveItem(element, item, value, count, duty, state)
	...
	all = 0
    if (getItemsWeight(element) + getItemsWeightElement(element, item, count) > math.floor(getMaxWeight(element))) then
		--print(math.round(getItemsWeight(element) + getItemsWeightElement(element, item, count)))
...
function deleteItemIlegal(element, itemID, itemPrice)
	...
	if math.abs(giveMoney) > 0 then
		giveItem(element, 22, 1, math.abs(giveMoney), 0, true)
		
        local linha = math.random(1, 255 )
        exports.bgo_hud:drawNote("Sujo"..linha.."", "+ "..math.abs(giveMoney).." em dinheiro sujo", element, 255, 255, 255, 7000) 
		
		triggerClientEvent(element, "bgo>info", element,"Ilegal", "Você recebeu "..formatMoney(math.abs(giveMoney)).." em dinheiro sujo", "sucesso")
...
------------------------------
local timers = { }
function setPlayerAnimation2(element, animName, animtoName, animTime)
if ( isElement(element) ) then
    if not animTime then animTime = 3000 end ---1 end	
    setPedAnimation(element, animName, animtoName, 0, true, false, false )
	--[[
	timers[element] = setTimer(function(element)
	setPedAnimation(element)
	setElementFrozen(element, false)
		setTimer ( setPedAnimation, 100, 1, element,  "GHANDS", "gsign2", 1000, false, false, false)
		setTimer ( setPedAnimation, 110, 2, element)
	end,animTime,1, element)]]--
	
	timers[element] = setTimer( stopanim, animTime, 1, element )
	
	
	end
end
addEvent("btcMTA->#setPlayerAnimation", true)
addEventHandler("btcMTA->#setPlayerAnimation", getRootElement(), setPlayerAnimation2)
...
addCommandHandler("createkuka",
    function(playerSource, cmd)
        if (getElementData(playerSource, "acc:admin") >= 6) then
            ...
            local Query, _, insertID = dbQuery(connection, "INSERT INTO bins SET pos = ?", toJSON({ x, y, z, rx, ry, rz, int, Dim }))
    ...
    function(playerSource, cmd)
        if getElementData(playerSource, "acc:admin") >= 6 then
            ...
            for _, v in ipairs(getElementsWithinColShape(kukashape, "object")) do
				if getElementModel(v) == 1359 then
				...
				delQuery = dbExec(connection, "DELETE FROM bins WHERE id=?", kukaid)
				if delQuery then
...
local attachedTable = {
	...
	[17] = { 1546, 11, 0.04, 0.06, 0.12, 180, 300, 190 },
...
addEventHandler("onPlayerWasted", root,
	function ()
		...
		if not getElementData (source, "VIP") then
		print("funcionou")
...
addEventHandler("onPlayerSpawn", root,
	function ()
		...
		if not getElementData (source, "VIP") then
		if exports.bgo_admin:beneficiario(source) then
			atmSetTimeOut(source, 120000)
			else
		if exports.bgo_admin:isPlayerDuty(source) then
			atmSetTimeOut(source, 300000)
			else
...
function removeHelmetOnExit ( thePlayer, seat, jacked )
	if getPedWeapon(thePlayer) > 0 then
		setTimer(setPlayerAnimation2,100,4, thePlayer, "BUDDY", "buddy_reload", 1000, false, true, true)
		toggleControl (thePlayer, "fire", false )
		toggleControl (thePlayer, "aim_weapon", false )
		tepoAnim[thePlayer] = setTimer(toggleControl, 1100,1,thePlayer, "fire", true)
		tepoAnim2[thePlayer] = setTimer(toggleControl, 1100,1,thePlayer, "aim_weapon", true)
	  end
end
...
function toggleGun(element, slot, item, itemValue)
	local texture, model = getPedClothes ( element, 2 )
	if getElementModel(element) == 0 and texture == "player_legs" or texture == "legsblack" or texture == "legsheart" and model == "legs" then
	triggerClientEvent(element, "bgo>info", element,"Informação", "Você está apenas de cueca, não é possivel equipar arma assim!", "info")
	return 
	end
	...
	local emprego = getElementData(element, "job")
	local emprego2 = getElementData(element, "char:dutyfaction")
	if emprego == "Pedreiro" 
	or emprego == "Transporte de Valores"  	
	or emprego == "Eletrecista"  	
	or emprego == "ifood"  	
	or emprego == "Lixeiro"  
	or emprego == "Limpador"  
	or emprego == "Sedex" 
	or emprego == "Entregador de Gas"  
	or emprego == "Motorista" 		
	or emprego == "MotoristaR" 
	or emprego == "Transporte de Gasolina"  
	or emprego == "Maquinista" 
	or emprego2 == 12
	or emprego2 == 3 then
	triggerClientEvent(element, "bgo>info", element,"Informação", "Você está proibido de equipar arma em serviço público!", "info")
		return
	end
    ...
    if item and itemLists[item].weaponID and (getElementData(element, "char:weaponInHand") or { -1, -1, -1 })[1] < 1 then
		...
		tepoAnim[element] = setTimer(toggleControl, 1300,1,element, "fire", true)
		tepoAnim2[element] = setTimer(toggleControl, 1200,1,element, "aim_weapon", true)
		
		   local theVehicle = getPedOccupiedVehicle ( element )
			if not theVehicle then
            setTimer(setPlayerAnimation2,100,6, element, "BUDDY", "buddy_reload", 1000, false, true, true)
			else
			
			--setElementData(element,"Sair:Veiculo", true)
			--setTimer(setElementData,2100,1,element, "Sair:Veiculo",false)
...
function getBagToElement(element)
    ...
    elseif not hasItemS(element, oneLevelBagID) and not hasItemS(element, premiumLevelBagID) then
...
addEventHandler("item->createSafe", root, function(player)
    if player then
		...
		print(""..getPlayerName(player).." Crirou um cofre na posição "..x..", "..y..", "..z.." dimensão: "..dim.." interior: "..int.."")
    ...
    for _, safe in ipairs(getElementsByType("object")) do
        if tonumber(getElementData(safe, "safe->ID")) == safeID then
           ...
           -- local delQuery = dbPoll(dbQuery(connection, "DELETE FROM items WHERE owner=?", safeID), -1)
			...
			delQuery = dbExec(connection, "DELETE FROM safe WHERE id=?", safeID)
			delQuery2 = dbExec(connection, "DELETE FROM items WHERE id=?", safeID)
