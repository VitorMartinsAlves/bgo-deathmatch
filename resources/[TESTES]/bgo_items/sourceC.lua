...
showmenu = { 'Cancelar', 'Mostrar', 'Usar'}
...
margin = 2 -- // 2 slot között lévő hely
...
itemSize = monitorSize[1]/39 -- // Itemkép méret

local width, height = itemSize * column + margin * (column + 1), itemSize * row + (1 + row) * margin + 60
...
local panelX, panelY = monitorSize[1] - width - 110, monitorSize[2] / 2 - height / 2 -- // Panel helyzete
--local panelX, panelY = sx*1400 - width - 110, sy*900 / 2 - height / 2 -- // Panel helyzete
...
bindKey('i', 'down', function()
    ...
    if not startCraft then
        ...
        if showInventory then
			...
			activeInvState = 'bag'
            craftTick = 0
            local som = playSound("files/sounds/open.mp3", false)
			setSoundVolume(som,0.3)
            setTimer(function()
            ...
            local som = playSound("files/sounds/close.mp3", false)
			setSoundVolume(som,0.3)
            setElementData(localPlayer, "playerInUse", false)
...
addEventHandler("onClientClick", root,
    function(button, state)
        if state == "down" and showInventory and isCursorShowing() then
...
addEventHandler("onClientCursorMove", root,
	...
	if showInventory and isCursorShowing() then
...
function mensagem(titulo, text, texto2)

triggerEvent("bgo>info", localPlayer,titulo, text, "erro")
					
--	focusBrowser(webBrowser)
--executeBrowserJavascript ( webBrowser, string.format("erroquant('%s', '%s')", titulo, text))
end
addEvent("inv:erro",true)
addEventHandler("inv:erro",root,mensagem)

function sucesso(text)

triggerEvent("bgo>info", localPlayer,"Boa!", text, "sucesso")
--focusBrowser(webBrowser)
--executeBrowserJavascript ( webBrowser, string.format("sucesso('%s')", text))

end
addEvent("inv:sucess",true)
addEventHandler("inv:sucess",root,sucesso)
...
addEventHandler('onClientRender', root, function()
    ...
    if not getElementData(localPlayer, "screen") and getElementData(localPlayer, "loggedin") == true then
        ...
        --dxDrawRectangle(actionPosX, actionPosY, actionWidth+tamanho-5, actionHeight+tamanho, tocolor(255, 136, 0, 200))
        ...
        for i = 1, actionbarSlot do
            dxDrawRectangle(actionPosX + margin - (itemSize + margin + tamanho) + i * (itemSize + margin + tamanho), actionPosY + margin, itemSize + tamanho, itemSize + tamanho, tocolor(0, 0, 0, 90))
			dxDrawText(i, actionPosX + margin - (itemSize + margin+tamanho) + i * (itemSize + margin+tamanho) + 5, actionPosY + margin + itemSize+tamanho - 17, itemSize+tamanho, itemSize+tamanho, tocolor(255, 255, 255, 155), 0.9, font, "left", "top", false, false, false, true)
			if isMouseInPosition(actionPosX + margin - (itemSize + margin + tamanho) + i * (itemSize + margin + tamanho), actionPosY + margin, itemSize+tamanho, itemSize+tamanho) then
                ...
                dxDrawRectangle(actionPosX + margin - (itemSize + margin + tamanho) + i * (itemSize + margin + tamanho), actionPosY + margin, itemSize+tamanho, itemSize+tamanho, tocolor(r, g, b, 255)) -- // ActionBar Item Hover
            else
            end
            if getKeyState(i) and not isCursorShowing() and not isChatBoxInputActive() or (actionBarTable[i] and actionBarTable[i]['itemID'] > -1 and getElementData(localPlayer, 'char:weaponGettin' .. actionBarTable[i]['tpyes'] .. actionBarTable[i]['slot']) or false) then
                dxDrawRectangle(actionPosX + margin - (itemSize + margin+tamanho) + i * (itemSize + margin+tamanho), actionPosY + margin, itemSize+tamanho, itemSize+tamanho, tocolor(255, 197, 118, 255)) -- // ActionBar Item Hover
            end
            if actionBarTable and actionBarTable[i] and actionBarTable[i]['itemID'] > -1 then
                 if (playerItemTable[actionBarTable[i]['tpyes']][actionBarTable[i]['slot']]['itemID'] or -1) > 0 then
                    dxDrawImage(actionPosX + margin - (itemSize + margin+tamanho) + i * (itemSize + margin+tamanho) + 2, actionPosY + margin + 2, itemSize+tamanho - 4, itemSize+tamanho - 4, getItemImg(actionBarTable[i]['itemID']), 0, 0, 0, tocolor(255, 255, 255, 255), false) --Item
                    if playerItemTable[actionBarTable[i]['tpyes']] and (playerItemTable[actionBarTable[i]['tpyes']][actionBarTable[i]['slot']]['count'] or 0) > 1 then
                        dxDrawText(playerItemTable[actionBarTable[i]['tpyes']][actionBarTable[i]['slot']]['count'], actionPosX + margin - (itemSize + margin+tamanho) + i * (itemSize + margin+tamanho) + 5, actionPosY + margin + itemSize+tamanho - 17, itemSize+tamanho, itemSize+tamanho, tocolor(0, 0, 0, 255), 1, font, "left", "top", false, false, false, true)
                        dxDrawText(playerItemTable[actionBarTable[i]['tpyes']][actionBarTable[i]['slot']]['count'], actionPosX + margin - (itemSize + margin+tamanho) + i * (itemSize + margin+tamanho) + 3, actionPosY + margin + itemSize+tamanho - 18, itemSize+tamanho, itemSize+tamanho, tocolor(255, 255, 255, 255), 1, font, "left", "top", false, false, false, true)
                    end
                else
                    dxDrawImage(actionPosX + margin - (itemSize + margin+tamanho) + i * (itemSize + margin+tamanho) + 2, actionPosY + margin + 2, itemSize+tamanho - 4, itemSize+tamanho - 4, 'img/no.png', 0, 0, 0, tocolor(255, 255, 255, 180), false) --Item
                end
            end
        end
    end
...
function drawInventory()
	...
	dxDrawRectangle(panelX, panelY + 30, width, height - 30, tocolor(0, 0, 0, 150))
	...
	--dxDrawRectangle(panelX + width - 64, panelY - 9, 60, 15, tocolor(255, 255, 255, 60))
	dxDrawRectangle(panelX + width - 66, panelY - 1, 64, 19, tocolor(0, 0, 0, 255))
    ...
    if isElement(btn_edit) then
        ...
        if tonumber(countText) then
            ...
            dxDrawText(countText, panelX + width - 66 + 64 / 2, panelY - 1 + 52 / 2, panelX + width - 66 + 64 / 2, panelY - 21 + 19 / 2, tocolor(255, 255, 255, 255), sy/1.3, 'default','center', 'center', false, false, false, true)
        else
            selectedAmount = 0
            dxDrawText("Numero", panelX + width - 66 + 64 / 2, panelY - 1 + 52 / 2, panelX + width - 66 + 64 / 2, panelY - 21 + 19 / 2, tocolor(255, 255, 255, 255), sy/1.3, 'default','center', 'center', false, false, false, true)
        end
    end

    if (isMouseInPosition(panelX, panelY, width, height)) and not confirmando  then
    ...
    for index, value in ipairs(inventoryElem) do
        if isMouseInPosition(panelX - 60, panelY + 255 - index * 50, itemSize, itemSize) and not confirmando  or value == activeInvState and not confirmando  then
		--dxDrawRectangle(panelX - 150 + width - index * 50, panelY - 15, 35, 35, tocolor(255, 255, 255, 150))
           dxDrawImage(panelX - 60, panelY + 255 - index * 50, itemSize, itemSize, 'img/' .. value .. '.png', 0, 0, 0, tocolor(255, 255,255, 255)) -- 40, 48			
        else
		--dxDrawRectangle(panelX - 150 + width - index * 50, panelY - 15, 35, 35, tocolor(255, 255, 255, 100))
            dxDrawImage(panelX - 60, panelY + 255 - index * 50, itemSize, itemSize, 'img/' .. value .. '.png', 0, 0, 0, tocolor(255, 255,255, 180)) -- 40, 48
        end
    end
    ...
    if activeInvState ~= 'craft' then
		...
		dxDrawText(texto, panelX + width / 2, panelY + height + 93 / 2, panelX + width / 2, panelY + height + 33 / 2, tocolor(0, 0, 0, 255), sy/1, 'pricedown', 'center', 'center', false, false, false, true)
        dxDrawText(texto, panelX + width / 2, panelY + height + 83 / 2, panelX + width / 2, panelY + height + 33 / 2, tocolor(255, 255, 255, 255), sy/1, 'pricedown', 'center', 'center', false, false, false, true)
        ...
        for i = 1, row * column do
            ...
            dxDrawRectangle(panelX + drawColumn * itemSize + (drawColumn + 1) * margin, panelY + (drawRow + 1) * margin + drawRow * itemSize + 30, itemSize, itemSize, tocolor(0, 0, 0, 120)) -- // Item BackGround

            if (itemTable[activeInvState]) then
                itemData = itemTable[activeInvState][i]
            end

            if isMouseInPosition(panelX + drawColumn * itemSize + (drawColumn + 1) * margin, panelY + (drawRow + 1) * margin + drawRow * itemSize + 30, itemSize, itemSize) and not confirmando  then
                if itemData and getItemSlot(i) > 0 then
                    tooltip_items(getSpecialItemName(itemData["itemID"], itemData["value"], itemData["count"]))
                end
                dxDrawRectangle(panelX + drawColumn * itemSize + (drawColumn + 1) * margin, panelY + (drawRow + 1) * margin + drawRow * itemSize + 30, itemSize, itemSize, tocolor(255, 197, 118, 255)) -- // Item Hover
            end
	...
	dxDrawRectangle(panelX, panelY + 30, width, height - 30, tocolor(0, 0, 0, 150))
		...
		dxDrawText("Criação de item", panelX + width / 2, panelY + height + 93 / 2, panelX + width / 2, panelY + height + 33 / 2, tocolor(0, 0, 0, 255), sx*1, 'pricedown', 'center', 'center', false, false, false, true)
        dxDrawText("Criação de item", panelX + width / 2, panelY + height + 83 / 2, panelX + width / 2, panelY + height + 33 / 2, tocolor(255, 255, 255, 255), sy*1, 'pricedown', 'center', 'center', false, false, false, true)



        dxDrawRectangle(panelX + width - 220, panelY + 36, 200, height - 60, tocolor(0, 0, 0, 100)) --- // Craft list background

        dxDrawRectangle(panelX + 95, panelY + height - 87, itemSize, itemSize, tocolor(255, 255, 255, 60)) -- // Giveitem BackGround
        ...
        if currentCraftItem > 0 then
            ...
            dxDrawImage(panelX + 95 + 2, panelY + height - 87 + 2, itemSize - 4, itemSize - 4, getItemImg(craftLists[currentCraftItem].giveCraftItem))
        ...
        for craftSlot = 1, maxCraftSlot do
            ...
            if currentCraftItemTable[craftSlot] and currentCraftItemTable[craftSlot][1] > 0 then
                ...
                if isMouseInPosition(panelX + drawColumn * itemSize + (drawColumn + 1) * margin + 25, panelY + (drawRow + 1) * margin + drawRow * itemSize + 35, itemSize, itemSize) and not confirmando  then
                    if hasState then
                        if playerItemTable[types][slot]['count'] >= currentCraftItemTable[craftSlot][2] then
        ...
        if isMouseInPosition(panelX + width - 215, panelY + height - 50, 190, 20) and not startCraft and not confirmando then
            dxDrawRectangle(panelX + width - 215, panelY + height - 50, 190, 20, tocolor(r, g, b, 220))
        end
    ...
    dxDrawRectangle(panelX + 5, panelY + height - 20, width - 10, 16, tocolor(255, 255, 255, 60)) 
    local weight = (width - 10) * getItemsWeight() / getMaxWeight(inventoryElement)
    if weight <= (width - 10) then
        dxDrawRectangle(panelX + 5, panelY + height - 20, weight, 16, tocolor(124, 197, 118, 255)) 
    else
        dxDrawRectangle(panelX + 5, panelY + height - 20, width - 10, 16, tocolor(124, 197, 118, 255)) 
    end
    dxDrawText(getItemsWeight() .. " / " .. tostring(getMaxWeight(inventoryElement)) .. " kg", panelX + width / 2, panelY + height - 20 + 16 / 2, panelX + width / 2, panelY + height - 20 + 16 / 2, tocolor(0, 0, 0, 255), 1, font, 'center', 'center', false, false, false, true)
		...
		if confirmando then
			dxDrawRectangle(panelX, panelY + 30, width, height - 30, tocolor(0, 0, 0, 150))
			for index, value in ipairs(showmenu) do
			if isMouseInPosition(posx, posy - index * 40, sx*80, sy*20) then
			dxDrawButton(value, posx, posy - index * 40, sx*80, sy*20, tocolor(255, 255, 255, 100), tocolor(0, 0, 0, 100))
			
			else
			dxDrawButton(value, posx, posy - index * 40, sx*80, sy*20, tocolor(255, 255, 255, 200), tocolor(0, 0, 0, 200))
				end
			end
		end
	end
...
local bgColor = tocolor(255, 255, 255,155)
function dxDrawButton(text, startX, startY, width, height, color, color2)
	...
	dxDrawText(text, startX+1, startY+1, startX + width, startY + height, tocolor(0, 0, 0,255), sx*1, "default", "center", "center", false, false, false, true)
	dxDrawText(text, startX, startY, startX + width, startY + height, color2, sx*1, "default", "center", "center", false, false, false, true)
...
function packetLossCheck()
    ...
    if (packet > 50) then
        ...
        packetTimer = setTimer(function() end, 1000 * random, 1)
		if showInventory then
		carregamento = false
		end
--[[
...
addEventHandler('onClientRender', root, packetLossCheck)
--setTimer(packetLossCheck, 70, 0)
...
function enterVehicle2(player, seat, jacked)
    if player == localPlayer then
        if showInventory then
		carregamento = false
            bodySearch = true
...
function enterVehicle2(player, seat, jacked)
	...
	if getPedWeapon(localPlayer) > 0 then
		...
		setTimer(toggleControl, 1100,1, "fire", true)
		setTimer(toggleControl, 1100,1, "aim_weapon", true)
		end
	end
end
addEventHandler("onClientVehicleExit", getRootElement(), enterVehicle2)
...
function clickInventory(button, state, _, _, _, _, _, element)
    if button == 'left' and state == 'down' and (not showInventory or showInventory) and (not cursorinInventory and cursorinActionbar) and not confirmando then
    ...
    elseif button == 'left' and state == 'down' and showInventory and not confirmando then
        if activeInvState == 'craft' then
            local elem = 0
            for index, value in ipairs(craftLists) do
                if (index > scroll and elem < maxCraftRecipe) then
        ...
        if isMouseInPosition(panelX + width - 66, panelY - 1, 64, 19) then
        ...
        if getElementType(inventoryElement) == 'player' then
            for index, value in ipairs(inventoryElem) do
                if isMouseInPosition(panelX - 60, panelY + 255 - index * 50, itemSize, itemSize) and not startCraft then
        ...
        if (cursorInSlot[2] > -1) and cursorinInventory and bodySearch then -- // Move Start
            ...
            timers1 = setTimer(function() end, 1000, 1) --- spam védelem
    ...
    elseif button == 'left' and state == 'up' and showInventory and not confirmando then
		if getElementData(localPlayer, "venderDrogas") then
        ...
        timer2 = setTimer(function() end, 1000, 1) --- spam védelem
		...
		--[[
            ...
            if (cursorinInventory and movedItem[1] > -1 and movedItem[2] > 0 and currentSlot[2] < 0 and currentSlot[2] ~= movedItem[2] and currentSlot[1] ~= movedItem[1] and currentSlot[1] > -1 and tonumber(selectedAmount) < 1 and bodySearch) then
                if getItemActionBar(movedItem[1]) then
                    ...
                    if not getElementData(inventoryElement, 'char:weaponGettin' .. getItemType(movedItem[2]) .. movedItem[1]) or false then
                        ...
                        delSlot(movedItem[1])
                        local som = playSound("files/sounds/mozgat.mp3", false)
						setSoundVolume(som,0.3)
                        movedItem = {}
            ...
            elseif (movedItem[1] > -1 and movedItem[2] > 0 and movedItem[3] and element ~= inventoryElement and isElement(element) and tostring(getElementType(element)) ~= "false" and not cursorinInventory) then
                if (getElementType(element) == "player" or getElementType(element) == "vehicle" or (getElementType(element) == "object" and getElementModel(element) == 2332)) then
                    ...
                    if isTimer(timers) then movedItem = {} return end
                    timers = setTimer(function() end, 1000, 1) --- spam védelem
                                                        ...
                                                        if not getElementData(element, "inUse") or false then
                                                            if not vehicleWeight[getElementModel(element)] then
                                                                if movedItem[2] == 144
																	...
																	antibug3 = setTimer(function() end,3000,1)
    ...
    elseif button == 'right' and state == 'down' and not showInventory and element and not confirmando then
        if getElementType(element) == "vehicle" and not cursorinInventory and not isMove and getElementData(element, "veh:owner") == getElementData(localPlayer, "char:id") then --or getElementData(localPlayer, "acc:admin") or 0) > 5  then --tonumber(getElementData(element, "dbid")) or -1 > 0 then
			...
			outputChatBox("#D24D57[bgo~Items] #ffffffAguarde 3 segundos para abrir o porta malas novamente", 255, 255, 255, true)
			return 
			end
                ...
                if (not getElementData(element, "inUse")) or false or (getElementData(element, "clickToVehicle") or 0) == 0 then
                    if not isPedInVehicle(localPlayer) then
                        if not isVehicleLocked(element) then
                            if not startCraft then
                               -- abininho
                                timervEH = setTimer(function() end, 3000, 1)
								antibug2 = setTimer(function() end,3000,1)
        ...
        elseif (getElementType(element) == "object" and getElementModel(element) == 2332 or getElementModel(element) == 1829 and (getElementData(element, "safe->ID") or 0) >= 1) then
            if hasItem(localPlayer, 19, getElementData(element, "safe->ID")) or (getElementData(localPlayer, "acc:admin") or 0) > 7 then
                --if isTimer(packetTimer) then return end
                if isTimer(timers) then movedItem = {} return end
                timers = setTimer(function() end, 1000, 1)
	...
	elseif state == "down" and button == "left" and confirmando then
	--if button == 'left' and state == 'up' and showInventory and bodySearch and confirmando then
    for index, value in ipairs(showmenu) do
	if isMouseInPosition(posx, posy - index * 40, sx*80, sy*20) then
	...
	confirmando = false
	movedItem = {}
	elseif value == "Usar" then
	useItem(UsarItem[1],UsarItem[2],UsarItem[3],UsarItem[4],UsarItem[5])
	confirmando = false
	UsarItem = {}
	movedItem = {}
	elseif value == "Mostrar" then
        local startTime = getTickCount()
        setElementData(localPlayer, "char >> showedItem", { true, UsarItem[3], getItemName(UsarItem[3]), getItemDescription(UsarItem[3]), getTickCount() })
        outputChatBox("Mostrou o item: #F7CA18" .. getElementData(localPlayer, "char >> showedItem")[3], 255, 255, 255, true)
        triggerServerEvent("btcMTA->#setPlayerMe", localPlayer, localPlayer, "Está mostrando um item (" .. getItemName(UsarItem[3]) .. ")")
        setTimer(function()
        setElementData(localPlayer, "char >> showedItem", { false, nil, nil, nil, nil })
        end, 5000, 1)
		UsarItem = {}
    ...
    elseif button == 'right' and state == 'up' and showInventory and bodySearch and not confirmando then
        if inventoryElement == localPlayer then
            if cursorInSlot[2] > 0 then
				...
				posx = math.max(10, math.min(x, sx -  width / 2 + 40))
				posy = math.max(10, math.min(y, sy -  height  / 2))
				confirmando = true
				movedItem = {}
				UsarItem = {itemTable[getItemType(cursorInSlot[2])][cursorInSlot[1]]['dbID'], cursorInSlot[1], cursorInSlot[2], cursorInSlot[3], cursorInSlot[4]} 
...
function isMouseInPosition( posX, posY, width, height )
