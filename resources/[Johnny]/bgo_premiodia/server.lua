...
function givePrize(oyuncu,hesap,kod,miktar,birim) -- ödülü veren fonksiyon
	...
	outputChatBox("Você recebeu seu presente diário! +#00FF00"..miktar..birim,oyuncu,150,150,150,true)
end
...
addEventHandler("prizeSystem:givePrize",root,function(day)
	...
	if not isGuestAccount(acc) then
		...
		outputChatBox("Você não pode receber seu presente diário sem fazer login na sua conta!",source,255,0,0,true)
	end
end)
