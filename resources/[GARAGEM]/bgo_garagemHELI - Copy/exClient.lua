...
parkYerleri2 = createMarker(1643.206, 1676.687, 10.82-0.8, "cylinder", 5, 0, 176, 196, 10 )
parkYerleri3 = createMarker(1426.7261962891,-1789.6497802734,12.555365562439, "cylinder", 5, 0, 176, 196, 10 )




local myBlip2 = createBlipAttachedTo ( parkYerleri2, 55 )
setElementData(myBlip2,"blipName", "Garagem Heli")
local myBlip3= createBlipAttachedTo ( parkYerleri3, 55 )
setElementData(myBlip3,"blipName", "Garagem Heli")
			...
			if getElementData(value, "veh:id") == tonumber(vehicleId) then			
				if not getElementData(value, "veh:owner") == getElementData(localPlayer, "char:id") then 
				   ...
				   return
					...
					triggerServerEvent("updateINTDIMGARAGEMHELI", localPlayer,localPlayer, vehicleId)
					else
...
function(button, state, _, _, _, _, _, element)
	if button == 'left' and state == 'down' and dashboardOpened then
			...
			triggerServerEvent("respawnVehicleHELI", localPlayer, localPlayer)
...
addEventHandler("onClientRender", root, function()
	if dashboardOpened then
		...
		local forY = startY + 2 * spacerBig + 200 + spacer * 22
		if isMouseInPosition(startX + 2 * spacerBig + spacer, forY, width / 2 - 4 * spacerBig - 2 * spacer, 20) then
		dxDrawButton("SPAWNAR", startX + 2 * spacerBig + spacer, forY, width / 2 - 4 * spacerBig - 2 * spacer, 20, tocolor(124, 197, 118, 100))
		else
		dxDrawButton("SPAWNAR", startX + 2 * spacerBig + spacer, forY, width / 2 - 4 * spacerBig - 2 * spacer, 20, tocolor(124, 197, 118, 180))
		end
		
		if isMouseInPosition(startX + 2 * spacerBig + spacer, forY+40, width / 2 - 4 * spacerBig - 2 * spacer, 20) then
		dxDrawButton("GUARDAR VEICULO ATUAL", startX + 2 * spacerBig + spacer, forY+40, width / 2 - 4 * spacerBig - 2 * spacer, 20, tocolor(124, 197, 118, 100))
		else
		dxDrawButton("GUARDAR VEICULO ATUAL", startX + 2 * spacerBig + spacer, forY+40, width / 2 - 4 * spacerBig - 2 * spacer, 20, tocolor(124, 197, 118, 180))
		end
