...
function EnviarMoney(item, targetplayer)
		...
		local targetPlayer2, targetPlayerName = exports.bgo_core:findPlayer(source, targetplayer)
...
function giveMoneyToPlayer(thePlayer, id,money)
	if not id then exports.bgo_infobox:addNotification(thePlayer,"Informe a conta da pessoa que deseja transferir","error") return end
	if not money then exports.bgo_infobox:addNotification(thePlayer,"Informe a quantidade para transferir","error") return end
	
	
	
	

	if not money == tonumber(money) then exports.bgo_infobox:addNotification(thePlayer,"apenas numeros","error") return end

	if not money == math.floor(money) then exports.bgo_infobox:addNotification(thePlayer,"apenas numeros redondo exemplo 2000 ","error") return end
		
	
	if isTimer(timer[thePlayer]) then
	exports.bgo_infobox:addNotification(thePlayer,"Aguarde 15 segundos!","error")
	outputChatBox('#0071fe[TRANSFERIDOR] #FFFFFFAguarde 15 segundos', thePlayer,255,255,255,true) 
	return
	end
...
if getElementData(thePlayer, "char:money") >= money then 
	...
	outputChatBox ('#FFFFFF R$:'..format(money)..' + R$:'..format(tonumber(getElementData(player2, "char:money") or 0))..' ultrapassa mais de 100 mil que o jogador tem, não foi possivel enviar', thePlayer, 200, 0, 0, true)
	exports.bgo_hud:dm('#FFFFFF R$:'..format(money)..' + R$:'..format(tonumber(getElementData(player2, "char:money") or 0))..' ultrapassa mais de 100 mil que o jogador tem, não foi possivel enviar',thePlayer, 200, 100, 0)


	outputChatBox('#FFFFFF R$:'..format(money)..' + R$:'..format(tonumber(getElementData(player2, "char:money") or 0))..' do jogador, ultrapassa mais de 100 mil e o maximo que você pode ter em suas mãos é a baixo de 100 mil', player2,255,255,255,true) 
	exports.bgo_hud:dm('#FFFFFF R$:'..format(money)..' + R$:'..format(tonumber(getElementData(player2, "char:money") or 0))..' do jogador, ultrapassa mais de 100 mil e o maximo que você pode ter em suas mãos é a baixo de 100 mil',player2, 200, 100, 0)
	return
	end
...
exports.bgo_hud:dm("Você enviou R$: "..money.." para " .. getPlayerName(player2),thePlayer, 200, 100, 0)
exports.bgo_hud:dm("*" .. getPlayerName(thePlayer) .." enviou R$: "..money.." para você",player2, 200, 100, 0)


--exports.logs:logMessage("[TRANSFERENCIA CELULAR]: "..getPlayerName(thePlayer).." ID: "..getElementData(thePlayer, "acc:id").." enviou via celular R$: "..money.." para "..getPlayerName(player2).." ID: "..getElementData(player2, "acc:id").." ", 1)	
...
exports.bgo_hud:dm("*Este jogador esta muito longe de você",thePlayer, 200, 100, 0)
end
else
exports.bgo_hud:dm("*Você pode enviar dinheiro pra você mesmo!",thePlayer, 200, 100, 0)
outputChatBox ( "#FFFFFF*Você pode enviar dinheiro pra você mesmo!", thePlayer , 200, 0, 0, true)
end
else
exports.bgo_hud:dm("*Este jogador não esta online!",thePlayer, 200, 100, 0)
outputChatBox ( "#FFFFFF*Este jogador não esta online!", thePlayer , 200, 0, 0, true)
end
end
addEvent("MoneyTransfer2",true)
addEventHandler("MoneyTransfer2",root,giveMoneyToPlayer)
...
addEventHandler("BTCdroid.startAnimation", root, function(thePlayer)
	--setPedAnimationProgress(thePlayer, "phone_in", 0.8)
...
function abrircelular()			
	...
	if counter[source] > 15 then
		exports.bgo_admin:outputAdminMessage("#ff0000[ATENÇÃO] #ffffff" .. getPlayerName(source) .. " (" .. getElementData(source, "playerid") .. ") Dentro de 15 segundos ele tentou abrir o celular 15 vezes.")
		counter[source] = 0
	end
...
addEventHandler("animarcelular", root, animarcelular)
 ...
 function painel(thePlayer)
	...
	triggerClientEvent(thePlayer, "Celular", thePlayer)
	else
...
--exports.bgo_discordlogs:sendDiscordMessage(1, false, "`[CHAMADA]: "..getPlayerName(thePlayer).." ID: "..getElementData(thePlayer, "acc:id").." Chamou mecanico`")
		...
		for theKey,player in ipairs (getElementsByType("player")) do
			if getElementData(player, "char:dutyfaction") == 4  then
			     triggerClientEvent(player, "chamada:dx", player, getPlayerName(thePlayer).." | "..(menCopom or "Sem descrição"))
...
function (thePlayer, menCopom)
	...
	for k,v in ipairs(getElementsByType("player")) do
        ...
        --if (prfCallCount > 0) then outputChatBox("#dc143c[BGOMTA - ERRO]:#ffffff já está uma chamada em andamento, Por favor aguarde e tente novamente.", thePlayer, 255, 255, 255, true) return end
		if getElementData(thePlayer, "call:prf") then outputChatBox("#dc143c[BGOMTA - ERRO]:#ffffff Você já tem uma chamada em andamento aguarde 1 minuto para fazer novamente.", thePlayer, 255, 255, 255, true) return end

		outputChatBox("#bebebeVocê chamou a Guarda Municipal aguarde.", thePlayer, 255, 255, 255, true)
		exports.bgo_hud:dm(" "..getPlayerName(thePlayer).." Você chamou a Guarda Municipal aguarde.", thePlayer, 255, 200, 0)

		prfCallCount = prfCallCount + 1
		prfCall[prfCallCount] = thePlayer
		setElementData(thePlayer, "call:prf", prfCallCount)

		setTimer ( setElementData, 60000, 1, thePlayer, "call:prf", false)
		...
		local x, y, z = getElementPosition(thePlayer)
		setElementData(thePlayer, "call:prfposx", x)
		setElementData(thePlayer, "call:prfposy", y)
		setElementData(thePlayer, "call:prfposz", z)
		for theKey,player in ipairs (getElementsByType("player")) do
			if getElementData(player, "char:dutyfaction") == 21  then
			     triggerClientEvent(player, "chamada:dx", player, getPlayerName(thePlayer).." | "..(menCopom or "Sem descrição"))
                 bindKey(player, "e","down", attChamada)
				 idChamada[player] = prfCallCount
			  end
		end
end )
	...
	if getElementData(thePlayer, "char:dutyfaction") == 21  then
		if (acceptID) then
			...
			if prfCall[acceptID] then
				...
				exports.bgo_hud:dm("A GM aceitou sua chamada e está a caminho", prfCall[acceptID], 255, 200, 0)
...
addCommandHandler("ac", aceitarprf)
...
function (thePlayer, menCopom)
	...
	--timer = setTimer(function() end, 10000, 1)
		...
		--exports.bgo_discordlogs:sendDiscordMessage(1, false, "`[CHAMADA]: "..getPlayerName(thePlayer).." ID: "..getElementData(thePlayer, "acc:id").." Chamou policia`")
...
--exports.bgo_discordlogs:sendDiscordMessage(1, false, "`[CHAMADA]: "..getPlayerName(thePlayer).." ID: "..getElementData(thePlayer, "acc:id").." Chamou staff`")
		...
		for theKey,player in ipairs (getElementsByType("player")) do
			if getElementData(player, "char:adminduty") == 1  then
			--	outputChatBox("#bebebe"..getPlayerName(thePlayer).." chamou o staff utilize /ac " .. staffCount .. " para aceitar o pedido", player, 255, 255, 255, true)
			--	displayServerMessage(player, ""..getPlayerName(thePlayer).." chamou  o staff utilize /ac " .. staffCount .. " para aceitar o pedido", "warning")
			triggerClientEvent(player, "chamada:dx", player, getPlayerName(thePlayer).." | "..((menCopom or "Chamada sem descrição")))
			    bindKey(player, "e","down", attChamada)
...
setTimer(function()
	...
	--	for theKey,player in ipairs (getElementsByType("player")) do
	--		if getElementData(player, "char:adminduty") == 1  then
	--			outputChatBox("#dc143c[BGOMTA - CHAMADA] #bebebeChamadas de mecanico alteradas para  "..mecCount.."   ", player, 255, 255, 255, true)
	--		end
	--	end
...
function( plr, message )
	...
	exports.logs:logMessage("[WHATSAPP] " .. getPlayerName(plr) .. " ("..getElementData(plr, "char:id")..") para " .. getPlayerName(source) .. " ("..getElementData(source, "char:id")..") : " .. message, 4)


	
	for k,v in ipairs(getElementsByType("player")) do
		if isElement(v) and getElementData(v, "loggedin") then --tonumber(getElementData(v,"acc:admin") or 0) == 10 then
		...
		outputChatBox("#7cc576[BGO MTA - LOG]:#ffffff[WHATSAPP] #7cc576" .. getPlayerName(plr) .. " ("..getElementData(plr, "char:id")..") #FFFFFFpara #7cc576" .. getPlayerName(source) .. " ("..getElementData(source, "char:id")..") : #FFFFFF" .. message,v,255,255,255,true)
		end
	end
	end
...
addEvent( "SendMsgToTeamMecanicoBlip", true )
addEventHandler( "SendMsgToTeamMecanicoBlip", root,
function ( )
				if isElement(blipMec[source]) then
				destroyElement(blipMec[source])
				end
				if isElement(blipPolicia[thePlayer]) then
				destroyElement(blipPolicia[thePlayer])
				end
...
function diss ( thePlayer )
			local x, y, z = getElementPosition(thePlayer)
			local tx, ty, tz = 1729.8291015625,497.52355957031,31.407165527344 --getElementPosition(player)
			local distance = getDistanceBetweenPoints3D(x, y, z, tx, ty, tz)
			
			outputChatBox("Distancia: "..math.floor(distance).."",thePlayer,255,255,255,true)
			
end
--addCommandHandler("dis", diss)			
