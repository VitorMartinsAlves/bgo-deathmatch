...
function createPanel()
	...
	for index, value in ipairs (celular_Table) do 
		...
		if value[1] == "Revistar Jogador" then 
			if getElementData(localPlayer, "char:adminduty") == 1 or getElementData(localPlayer, "char:dutyfaction") == 27 or getElementData(localPlayer, "char:dutyfaction") == 18 or getElementData(localPlayer, "char:dutyfaction") == 7 or getElementData(localPlayer, "char:dutyfaction") == 25 or getElementData(localPlayer, "char:dutyfaction") == 23 or getElementData(localPlayer, "char:dutyfaction") == 24 or getElementData(localPlayer, "char:dutyfaction") == 16 or getElementData(localPlayer, "char:dutyfaction") == 8 or getElementData(localPlayer, "char:dutyfaction") == 10 or getElementData(localPlayer, "char:dutyfaction") == 13 or getElementData(localPlayer, "char:dutyfaction") == 14 or getElementData(localPlayer, "char:dutyfaction") == 15 or getElementData(localPlayer, "char:dutyfaction") == 26 then
...
function teste(button, state, x, y, elementx, elementy, elementz, element)
	if state == "down" and button == "left" and inicio then 	
		...
		for index, value in ipairs (celular_Table) do 
			...
			if value[1] == "Ligações" then 
			if tonumber(getElementData(localPlayer,"char:telefone") or 0) > 0 then
			exports.mta_phone:showPhoneFunction(getElementData(localPlayer,"char:telefone"))
			print("ABRIU")
			else
			outputChatBox("#7cc576Você não tem um numero de telefone registrado para registrar um numero digite /reconnect e reloga na cidade!", 255, 255, 255, true)
			end
			end
				...
				for _, player in ipairs(getElementsByType("player")) do
					...
					if distance <= 1 then
						...
						if player ~= localPlayer then
			    			 if getElementData(localPlayer, "char:adminduty") == 16 or getElementData(localPlayer, "char:dutyfaction") == 27 or getElementData(localPlayer, "char:dutyfaction") == 23 or getElementData(localPlayer, "char:dutyfaction") == 25 or getElementData(localPlayer, "char:dutyfaction") == 24 or getElementData(localPlayer, "char:adminduty") == 1 or getElementData(localPlayer, "char:dutyfaction") == 7 or getElementData(localPlayer, "char:dutyfaction") == 18 or getElementData(localPlayer, "char:dutyfaction") == 8 or getElementData(localPlayer, "char:dutyfaction") == 10 or getElementData(localPlayer, "char:dutyfaction") == 13 or getElementData(localPlayer, "char:dutyfaction") == 14 or getElementData(localPlayer, "char:dutyfaction") == 15 or getElementData(localPlayer, "char:dutyfaction") == 26 then
			...
			if (index2 > nextPage and elem2 < maxElem) then
				...
				triggerServerEvent("MoneyTransfer2",localPlayer, localPlayer, targerplayer, item);
			...
			if (index > nextPage and elem < maxElem) then
				...
				fac = "Policia"
				--exports.dgs:dgsSetVisible(boxEmergencia, true)
				
				if isElement(boxEmergencia) then
				windowClosed()
				end
				boxEmergencia = exports.dgs:dgsCreateEdit(sx*509, sy*439, sx*375, sy*77, "", false)  
				exports.dgs:dgsEditSetMaxLength ( boxEmergencia, 20 )    
                ...
                OpenWin()		
				elseif value[1] == "Guarda Municipal" then
				fac = "GM"
				--exports.dgs:dgsSetVisible(boxEmergencia, true)
			...
			if (index > nextPage and elem < maxElem) then
				...
				if ( isTimer( tempodinheiro ) ) then return end
				tempodinheiro = setTimer( function(  ) end, 1500, 1 )
...
function windowClosed()
		cancelEvent()
		--exports.dgs:dgsSetVisible (boxEmergencia, false)
		destroyElement( boxEmergencia ) 
end
...
function(button, state, x, y, elementx, elementy, elementz, element)
	--if state == "down" and boxMenStats then  
		if boxMenStats then
             getMen = exports.dgs:dgsGetText (boxEmergencia)
             if (getMen == "") then
			     outputChatBox("#FFA000*BGO ERROR #FFFFFFAdicione um descrição para chamada.", 255,255,255, true)
			     return
			 end 
		         if (fac == "Policia") then
				     triggerServerEvent('SendMsgToTeamPolicia', localPlayer, localPlayer, getMen)
                     fac = nil	
                     boxMenStats = false	
				    ...
				    -- return
		        elseif (fac == "GM") then
				     triggerServerEvent('SendMsgToTeamPRF', localPlayer, localPlayer, getMen)
                     removeEventHandler("onClientRender", root, boxMen)							 
					 fac = nil	
					 boxMenStats = false
					-- exports.dgs:dgsSetVisible(boxEmergencia, false)
		        ...
		        elseif (fac == "BGO - Resgate") then
				     triggerServerEvent('SendMsgToTeammedic', localPlayer, localPlayer, getMen)
                     removeEventHandler("onClientRender", root, boxMen)							 
					 fac = nil	
					 boxMenStats = false
					 --guiSetInputEnabled(false)
...
function closeWin()
	...
	semcelulardinheiro = false
		...
		local Block, Anim = getPedAnimation(localPlayer) -- get the player animation
		--if not Block then
		print(Block)
		triggerServerEvent("tiraranimacao", localPlayer, localPlayer) 		
		--else
...
addEventHandler("CelularOFF", root, closeWin)
...
function createPoliciaMarker(player, id, tplayer, x, y, z)
	if isElement(player) then
		...
		setElementData(Policia[id],"blip >> blipOnScreen", true)
...
function emergencyMarker( hitPlayer, matchingDimension )
	if PoliciaMarkers[getElementData(source, "call:id")] and getElementData(source, "call:accepted") == hitPlayer then
		...
		if (acceptID) then
			exports.bgo_hud:dm("Você chegou no chamado " .. acceptID .. " Faça seu serviço.", 255, 200, 0)
			triggerServerEvent('SendMsgToTeamMecanicoBlip', localPlayer, localPlayer)
			--setElementData(tplayer, "call:policia", false)
...
setElementData(medicoblip[id],"blip >> blipOnScreen", true)
...
function medicomarker( hitPlayer, matchingDimension )
	if MedicoMarkers[getElementData(source, "call:id")] and getElementData(source, "call:accepted") == hitPlayer then
		...
		if (acceptID) then
			exports.bgo_hud:dm("Você chegou no chamado " .. acceptID .. " Faça seu serviço.", 255, 200, 0)
			triggerServerEvent('SendMsgToTeamMecanicoBlip', localPlayer)
			--setElementData(tplayer, "call:medico", false)
...
setElementData(mecanicoblip[id],"blip >> blipOnScreen", true)
...
setElementData(taxiblip[id],"blip >> blipOnScreen", true)
...
function taximarker( hitPlayer, matchingDimension )
	if taxiMarkers[getElementData(source, "call:id")] and getElementData(source, "call:accepted") == hitPlayer then
		...
		if (acceptID) then
			...
			--setElementData(tplayer, "call:taxi", false)
...
setElementData(detranblip[id],"blip >> blipOnScreen", true)
...
function detranmarker( hitPlayer, matchingDimension )
	if detranMarkers[getElementData(source, "call:id")] and getElementData(source, "call:accepted") == hitPlayer then
		...
		if (acceptID) then
			exports.bgo_hud:dm("Você chegou no chamado " .. acceptID .. " Faça seu serviço.", 255, 200, 0)
			triggerServerEvent('SendMsgToTeamMecanicoBlip', localPlayer)
			--setElementData(tplayer, "call:detran", false)
...
chat_Windows[player] = { };
chat_Windows[player].window = guiCreateWindow((screenW - 929) / 2, (screenH - 314) / 2, 629, 314, getPlayerName( player ), false)
...
function sendMessage( eleEdit )
    if boxMenStats then return end
	local parent = exports.dgs:dgsGetParent( source )
		if ( parent == false or not parent ) then return end
		    val = guiGetText ( parent )
			local player = getPlayerFromName( val )
		if ( player == false or not player ) then return end
