...
function detectAim( target )
    if getPlayerTeam(localPlayer) == getTeamFromName("Policia") then return end	
		...
		if isElementWithinColShape(player, teste1) then
		...
		setElementPosition(localPlayer,1348.8060302734,-1760.6199951172,13.549824714661)
		setElementRotation(localPlayer, 0,0, 181.26121520996)
		setElementFrozen(localPlayer, true)
		...
		outputChatBox("#dc143c[AVISO]:#ffffff Não tem policia na cidade ( minimo 20 policiais ), vaza daqui!", 255, 255, 255, true)
        end
		...
		local count = #getAlivePlayersInTeam(getTeamFromName ( "Policia" )) or 0
		triggerEvent("bgo>info", localPlayer,"Informação!", " "..tonumber(count).." ", "info")
		outputChatBox("#dc143c[AVISO]:#ffffff Não tem policia na cidade ( minimo 20 policiais ), vaza daqui!", 255, 255, 255, true)
		end	
