...
allowedTypes = { "Automobile", "Boat", "Train" }
...
function cc()
	...
	if tonumber(getElementData(getPedOccupiedVehicle(localPlayer),"veh:fuel") or 0) < 1 then	
		removeEventHandler("onClientRender", getRootElement(), cc)
		ccEnabled=false

		outputChatBox( "#7cc576[Piloto automatico]:#ffffff Desativado.", 255, 255, 255, true )
		return false
	end
...
bindKey(key, "up", function()
	if getElementData(localPlayer, "job") ~=  "Motorista" then
				...
				if targetSpeed > 4 then
					...
					if getVehicleEngineState ( veh ) then
						if in_array(getVehicleType(veh), allowedTypes) then
							...
							outputChatBox( "#7cc576[Piloto automatico]:#ffffff Piloto automatico ativado em: #7CC576 "..math.floor(targetSpeed).." #ffffff #7cc576km/h#ffffff", 255, 255, 255, true )
						...
						--outputChatBox( "#7cc576[ExtendGaming - tempomat]:#ffffff Tempomat aktiválva: #7cc576 "..targetSpeed.." #ffffff #7cc576km/h#ffffff", 255, 255, 255, true )
						addEventHandler("onClientRender", getRootElement(), cc)
						ccEnabled=true
						end
					end
