...
local gMe   = getLocalPlayer(); -- változó
local gRoot = getRootElement(); -- változó
...
local imgDims = { 
  { screen[1]-screen[1]/1.76+25, screen[2]-(screen[2]/2.5), screen[1]/1.76, screen[2]/1.6 }, -- pozíció
  { screen[1]/2-screen[1]/1.76, screen[2]-(screen[2]/1.8), screen[1]/1.76, screen[2]/1.6 }, -- pozíció
  { screen[1]-screen[1]/1.99, screen[2]-(screen[2]/1.5), screen[1]/1.76, screen[2]/1.6 }, -- pozíció
  { screen[1]-screen[1]/3.6, screen[2]/120, screen[1]/4.2, screen[2]/1.5 }, -- pozíció
  { screen[1]/25, screen[2]/25, screen[1]/1.76, screen[2]/1.6 }, -- pozíció
  { screen[1]/2-screen[1]/5, screen[2]/120, screen[1]/1.76, screen[2]/1.6 } -- pozíció
};
...
addEventHandler( 'onClientResourceStart', gRoot,
  function ( res )
    if res == getThisResource() then
      for i = 1, 6 do
        imgTable.image[i] = 'blood/files/comy_'..tostring(i)..'.png'; -- Kép beolvasása
        imgTable.alpha[i] = 0;
      end;
      rectangleAlpha = 0;
    end;
...
function renderRectangle()
  if rectangleAlpha > 0 then
    ...
    removeEventHandler( 'onClientRender', gRoot, renderRectangle ); -- vége
    drawRec = false;
  end;
end;