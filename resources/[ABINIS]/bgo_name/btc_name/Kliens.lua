--Scriptet Írta: Csoki
--BTCMTA 2017

if fileExists("Kliens.lua") then
	fileDelete("Kliens.lua")
end
if fileExists("Kliens.luac") then
	fileDelete("Kliens.luac")
end


local x, y = guiGetScreenSize()


local settings = {}
settings.font = dxCreateFont("files/roboto.ttf", 20)
settings.maxdis = 20 -- max távolság
settings.anames = false

local levelNames = {
	"#7CC576(Suporte em TESTE)", 
	"#7CC576(SUPORTE)",
	"#7CC576(MODERADOR)",
	"#7CC576(ADMINISTRADOR)", 
	"#7CC576(SUB GAME MASTER)",
	"#FF9600(GAME MASTER)",
	"#8A2BE2(SUPER GAME MASTER)",
	"#00BFFF<SCRIPTER/>", 
	"#FF0000(1ª DAMA)", 
	"#C25151(DEUS)"
}
levelNames[0] = ""

function clamp(val, lower, upper)
    if (lower > upper) then lower, upper = upper, lower end
    return math.max(lower, math.min(upper, val))
end


local helperNames = {"#C71585(Helper)", "#C71585(Helper)"}
helperNames[0] = ""

function getAdminLevelName(player)
	if getElementData(player, "char:adminduty") == 0 then return "" end
	return levelNames[getElementData(player, "acc:admin")]
end

function getHelperLevelName(player)
	return helperNames[getElementData(player, "acc:aseged")]
end


									--[[
									local posX, posY, posZ = getElementPosition(v)
									local posZ = posZ-0.9
									local radius, width, angleAmount = 0.5, 0.1, 0.1
									local startAngle = clamp(0, 0, 360)
							
									local endAngleHealth = 360*(100/100)
									local endAngleHealth = clamp(endAngleHealth or 360, 0, 360)
									local colorHealth = tocolor(0, 72, 255, 60)
							
								
									for i=startAngle,endAngleHealth,angleAmount do
										local _i = i*(math.pi/180)
										dxDrawLine3D(math.cos(_i)*(radius-width)+posX, math.sin(_i)*(radius-width)+posY, posZ, math.cos(_i)*(radius+width)+posX, math.sin(_i)*(radius+width)+posY, posZ, colorHealth, width, false)
									end]]--

local alpha = 200
addEventHandler("onClientRender",root, function()

	local x1,y1,z1 = getElementPosition(localPlayer)
	
	for k,v in ipairs(getElementsByType("player", _, true)) do
		if isElement(v) and isElementOnScreen(v) then
			if v ~= localPlayer then

				if not getElementData(v, "loggedin") then return end

				setPlayerNametagShowing (v, false)

				local x2,y2,z2 = getElementPosition(v)
				local cx,cy,cz = getCameraMatrix()
				local headPosX,headPosY,headPosZ = getPedBonePosition(v,4)
				local distance = getDistanceBetweenPoints3D(x1,y1,z1,x2,y2,z2)
				local distance = distance-(distance/3)
				
				local progress = distance/settings.maxdis
				local scale = interpolateBetween(0.7, 0, 0, 0.2, 0, 0, progress, "OutQuad")
				local scale = scale*(x+1920)/(1920*1.1)
				
				local clear = isLineOfSightClear(cx,cy,cz,headPosX,headPosY,headPosZ,true,false,false,true,false,false,false)
				
				local adminduty = getElementData(localPlayer, "char:adminduty")
				local adminlevel = getElementData(v, "acc:admin")
				local adminhelper = getElementData(v, "acc:aseged")
				local adminduty = getElementData(v, "char:adminduty")
				local adutyText = " "..getAdminLevelName(v)
				local helperText = " "..getHelperLevelName(v)
				local tazed = getElementData(v,"lesokkolt")
				
				if distance < settings.maxdis then
					local sx,sy = getScreenFromWorldPosition(x2,y2,headPosZ+0.38)
					if sy then
						sy = sy-15
					end
					local name = getPlayerName(v)
					if name then
						local nameformat = string.gsub(string.gsub(name, "#%x%x%x%x%x%x", ""),"_", " ")
						local id = getElementData(v, "playerid")
						
						if adminlevel > 0 then
							text = adutyText.."\n("..id..")"
						else
							text = helperText.."\n("..id..")"
						end
						
						local badge = getElementData(v, "jelveny:state")
						local badgeText = ""
						if badge then
							if sy then
								sy = sy-20
							end
							badgeText = "\n#F39C12("..getElementData(v, "jelveny:text")..")"
						end

						local colorAlpha = 255
						if getElementHealth(v) == 0 then
							colorAlpha = 80
						end

						if sx and sy and name and id then
							if not settings.anames and clear then
							
								
--[[
						if getElementData(localPlayer, "char:dutyfaction") == 4 or getElementData(localPlayer, "char:dutyfaction") == 6 or getElementData(localPlayer, "char:dutyfaction") == 2 or getElementData(localPlayer, "char:dutyfaction") == 11 or getElementData(localPlayer, "char:dutyfaction") == 19 or getElementData(localPlayer, "char:dutyfaction") == 5 or getElementData(localPlayer, "char:dutyfaction") == 16 or getElementData(localPlayer, "char:dutyfaction") == 20 or getElementData(localPlayer, "char:dutyfaction") == 21  then
							if getElementData(v, "char:adminduty") == 0 then
							dxDrawText(string.gsub(text, "#%x%x%x%x%x%x", ""), sx+1, sy+3, sx, sy, tocolor(0,0,0,255), scale, settings.font, "center", "top", false, false, false, true)
							dxDrawText(text, sx, sy+2, sx, sy, tocolor(247, 255, 0,colorAlpha), scale, settings.font, "center", "top", false, false, false, true)
						end
					end
					]]--



								if(getElementData(v, "VoiceCor")) then
									if getElementData(v, "char:adminduty") == 0 then
									dxDrawText("FALANDO", sx+1, sy+1, sx, sy, tocolor(0,0,0,255), scale-0.6, settings.font, "center", "top", false, false, false, true)
									dxDrawText("FALANDO", sx, sy, sx, sy, tocolor(247, 255, 0,colorAlpha), scale-0.6, settings.font, "center", "top", false, false, false, true)
									--dxDrawText(string.gsub(text, "#%x%x%x%x%x%x", ""), sx+1, sy+3, sx, sy, tocolor(0,0,0,255), scale, settings.font, "center", "top", false, false, false, true)
									--dxDrawText(text, sx, sy+2, sx, sy, tocolor(247, 255, 0,colorAlpha), scale, settings.font, "center", "top", false, false, false, true)
								end
							end

								if getElementData(v, "char:adminduty") == 1 and not getElementData(v, "reconx") then
									dxDrawText(string.gsub(text, "#%x%x%x%x%x%x", ""), sx+1, sy+1, sx, sy, tocolor(0,0,0,255), scale-0.5, settings.font, "center", "top", false, false, false, true)
									dxDrawText(text, sx, sy, sx, sy, tocolor(255,255,255,colorAlpha), scale-0.5, settings.font, "center", "top", false, false, false, true)
									if progress < 1 then
										local scale = interpolateBetween(0.7, 0, 0, 0, 0, 0, progress, "OutQuad")
										local width, height = 210*scale, 190*scale
										local sx, sy = sx-width/2, sy-height
										
										if alpha < 255 then
											alpha = alpha +1
										elseif alpha == 255 then
											alpha = 200
										end
										
										local adminlevel = getElementData(v, "acc:admin")
										
										local logoPath = "files/admin.png"
										--[[
										if adminlevel == 7 then
											logoPath = "files/sa.png"
										elseif adminlevel == 8 then
											logoPath = "files/dev.png"
										elseif adminlevel >= 10 then
											logoPath = "files/owner.png"
										end]]--
										dxDrawImage(sx, sy, width, height, logoPath, 0, 0, 0, tocolor(255,255,255,alpha))
									end
									end	





								if getElementData(localPlayer, "char:adminduty") == 1 then
								if not getElementData(v, "invisible") then	
									if getElementData(v, "char:adminduty") == 0 then
 									dxDrawText(string.gsub(text, "#%x%x%x%x%x%x", ""), sx+1, sy+1, sx, sy, tocolor(0,0,0,255), scale, settings.font, "center", "top", false, false, false, true)
									dxDrawText(text, sx, sy, sx, sy, tocolor(255,255,255,colorAlpha), scale, settings.font, "center", "top", false, false, false, true)
								end
								end
								dxDrawText(string.gsub(badgeText, "#%x%x%x%x%x%x", ""), sx+1, sy+1, sx, sy, tocolor(0,0,0,255), scale, settings.font, "center", "top", false, false, false, true)
								dxDrawText(badgeText, sx, sy, sx, sy, tocolor(255,255,255,255), scale, settings.font, "center", "top", false, false, false, true)
								end
								





								
								--[[
								if not getElementData(v, "invisible") then
									dxDrawText(string.gsub(text, "#%x%x%x%x%x%x", ""), sx+1, sy+1, sx, sy, tocolor(0,0,0,255), scale, settings.font, "center", "top", false, false, false, true)
									dxDrawText(text, sx, sy, sx, sy, tocolor(255,255,255,colorAlpha), scale, settings.font, "center", "top", false, false, false, true)
								
									dxDrawText(string.gsub(badgeText, "#%x%x%x%x%x%x", ""), sx+1, sy+1, sx, sy, tocolor(0,0,0,255), scale, settings.font, "center", "top", false, false, false, true)
									dxDrawText(badgeText, sx, sy, sx, sy, tocolor(255,255,255,255), scale, settings.font, "center", "top", false, false, false, true)
								end]]--
								
								
								
							elseif settings.anames then
								if (not getElementData(v, "invisible")) or (getElementData(v, "invisible") and getElementData(localPlayer, "acc:admin") >= 6) then
									dxDrawText(string.gsub(text, "#%x%x%x%x%x%x", ""), sx+1, sy+1, sx, sy, tocolor(0,0,0,255), scale, settings.font, "center", "top", false, false, false, true)
									dxDrawText(string.gsub(text, "#%x%x%x%x%x%x", ""), sx, sy, sx, sy, tocolor(255,0,0,255), scale, settings.font, "center", "top", false, false, false, true)
									
									dxDrawText(string.gsub(badgeText, "#%x%x%x%x%x%x", ""), sx+1, sy+1, sx, sy, tocolor(0,0,0,255), scale/10, settings.font, "center", "top", false, false, false, true)
									dxDrawText(badgeText, sx, sy, sx, sy, tocolor(255,255,255,255), scale/10, settings.font, "center", "top", false, false, false, true)
								end
							end
							
							if settings.anames then
								if (not getElementData(v, "invisible")) or (getElementData(v, "invisible") and getElementData(localPlayer, "acc:admin") >= 6) then
									local scale = interpolateBetween(0.7, 0, 0, 0, 0, 0, progress, "OutQuad")
									local width, height = 200*scale, 100*scale
									local sx, sy = sx-width/2, sy-height

									dxDrawRectangle(sx, sy+height-height/10, width*0.45, height/10, tocolor(0,0,0,200))
									dxDrawRectangle(sx, sy+height-height/10, width*0.45/100*getElementHealth(v), height/10, tocolor(255,255,0,180))
									
									dxDrawRectangle(sx+width*0.55, sy+height-height/10, width*0.45, height/10, tocolor(0,0,0,200))
									dxDrawRectangle(sx+width*0.55, sy+height-height/10, width*0.45/100*getPedArmor(v), height/10, tocolor(82,179,217,180))
								end
							end
						
--[[
							if clear then
								
								if getElementData(v, "afk") and not getElementData(v, "reconx") then
									if progress < 1 then
										local scale = interpolateBetween(0.7, 0, 0, 0, 0, 0, progress, "OutQuad")
										local width, height = 256*scale, 256*scale
										local sx, sy = sx-width/2, sy-height
										dxDrawImage(sx, sy, width, height, "files/afk.png")
									end
								elseif getElementData(v, "pmTyping") and not getElementData(v, "reconx") then
									if progress < 1 then
										local scale = interpolateBetween(0.7, 0, 0, 0, 0, 0, progress, "OutQuad")
										local width, height = 256*scale, 256*scale
										local sx, sy = sx-width/2, sy-height										
										dxDrawImage(sx, sy, width, height, "files/typing.png")
									end
								elseif getElementData(v, "consoleTyping") and not getElementData(v, "reconx") then
									if progress < 1 then
										local scale = interpolateBetween(0.7, 0, 0, 0, 0, 0, progress, "OutQuad")
										local width, height = 256*scale, 256*scale
										local sx, sy = sx-width/2, sy-height										
										dxDrawImage(sx, sy, width, height, "files/console.png")
									end
								elseif adminduty > 0 and not getElementData(v, "reconx") then
									if progress < 1 then
										local scale = interpolateBetween(0.7, 0, 0, 0, 0, 0, progress, "OutQuad")
										local width, height = 100*scale, 100*scale
										local sx, sy = sx-width/2, sy-height
										
										if alpha < 255 then
											alpha = alpha +1
										elseif alpha == 255 then
											alpha = 200
										end
										
										local adminlevel = getElementData(v, "acc:admin")
										
										local logoPath = "files/admin.png"
										if adminlevel == 7 then
											logoPath = "files/sa.png"
										elseif adminlevel == 8 then
											logoPath = "files/dev.png"
										elseif adminlevel >= 10 then
											logoPath = "files/owner.png"
										end
										
										
										dxDrawImage(sx, sy, width, height, logoPath, 0, 0, 0, tocolor(255,255,255,alpha))
									end		
									
							
								elseif tazed then
									if progress < 1 then
										local scale = interpolateBetween(0.7, 0, 0, 0, 0, 0, progress, "OutQuad")
										local width, height = 100*scale, 100*scale
										local sx, sy = sx-width/2, sy-height
										
										if alpha < 255 then
											alpha = alpha +1
										elseif alpha == 255 then
											alpha = 200
										end
										
										dxDrawImage(sx, sy, width, height, "files/taz.png", 0, 0, 0, tocolor(255,255,255,255))
									end
								end
							end
							]]--
						end
					end
				end
			end
		end
	end
	
	for k,v in ipairs(getElementsByType("ped", _, true)) do
		if isElement(v) and isElementOnScreen(v) then
			if v ~= localPlayer then
				--setPlayerNametagShowing (v, false)

				local x2,y2,z2 = getElementPosition(v)
				local cx,cy,cz = getCameraMatrix()
				local headPosX,headPosY,headPosZ = getPedBonePosition(v,4)
				local distance = getDistanceBetweenPoints3D(x1,y1,z1,x2,y2,z2)
				local distance = distance-(distance/3)
				
				local progress = distance/settings.maxdis
				local scale = interpolateBetween(0.7, 0, 0, 0.2, 0, 0, progress, "OutQuad")
				local scale = scale*(x+1920)/(1920*1.9)
				
				local clear = isLineOfSightClear(cx,cy,cz,x2,y2,headPosZ,true,false,false,true,false,false,false)
				
				if distance < settings.maxdis then
					local sx,sy = getScreenFromWorldPosition(x2,y2,headPosZ+0.38)
					if sy then
						sy = sy-15
					end
					local name = getElementData(v, "Ped:Name")
					if not name then return end
					
					local nameformat = string.gsub(string.gsub(name, "#%x%x%x%x%x%x", ""),"_", " ")
		

					local text = nameformat.." #f2921e[NPC]"

					local colorAlpha = 255
					if getElementHealth(v) == 0 then
						colorAlpha = 80
					end

					if sx and sy and name and clear then
						dxDrawText(string.gsub(text, "#%x%x%x%x%x%x", ""), sx+1, sy+1, sx, sy, tocolor(0,0,0,255), scale, settings.font, "center", "top", false, false, false, true)
						dxDrawText(text, sx, sy, sx, sy, tocolor(255,255,255,colorAlpha), scale, settings.font, "center", "top", false, false, false, true)
					end
				end
			end
		end
	end
end)

--[[
bindKey("z", "both",
    function(key, state)
		if state == "down" then
			
	if getElementData(localPlayer, "VoiceCor") == false then
	setElementData(localPlayer, "VoiceCor", true)
	end

        else
	setElementData(localPlayer, "VoiceCor", false)


    end
end
)
]]--

addEventHandler("onClientPlayerVoiceStart",root,function()
    if (source and isElement(source) and getElementType(source) == "player") and localPlayer == source then 
	--if getElementData(source, "VoiceCor") == false then
		setElementData(source, "VoiceCor", true)
	--	end
	end
end)

addEventHandler("onClientPlayerVoiceStop",root,function()
	if (source and isElement(source) and getElementType(source) == "player") and localPlayer == source then 
	--if getElementData(source, "VoiceCor") == true then
		setElementData(source, "VoiceCor", false)
	--end
end
end)




bindKey("mouse2", "both",
	function(key, state)
		
		--if getElementData(localPlayer, "char:adminduty") == 1 then
		if state == "down" then
			local pedSlot = getPedWeaponSlot ( localPlayer )
			if (pedSlot == 0) then
			if getElementData(localPlayer, "apontararma") == false then
				local theVehicle = getPedOccupiedVehicle ( localPlayer )
				if theVehicle then return end
			setElementData(localPlayer, "apontararma", true)
			triggerServerEvent ( "dararma", localPlayer )
			showPlayerHudComponent ("crosshair", false)
			setControlState ("fire", false)
			--toggleControl ( "fire", false ) 
			end
			end
	else
		if getElementData(localPlayer, "apontararma") == true then
			local theVehicle = getPedOccupiedVehicle ( localPlayer )
			if theVehicle then return end

		triggerServerEvent ( "removerarma", localPlayer )
		setElementData(localPlayer, "apontararma", false)

		setControlState ("fire", false)
   --setControlState ("fire", false)
   setPedWeaponSlot (localPlayer,  0 )
   showPlayerHudComponent ("crosshair", true)
   end 


end
end
)


function updateCamera ()
	if getElementData(localPlayer, "apontararma") then
		toggleControl ( "fire", false ) 
		setControlState ("fire", false)
	end
end
setTimer ( updateCamera, 70, 0 )




txd = engineLoadTXD ( "weapon.txd" )
engineImportTXD ( txd, 346 )
dff = engineLoadDFF ( "weapon.dff" )
engineReplaceModel ( dff, 346 )



bindKey("x", "both",
    function(key, state)
		if state == "down" then
			local theVehicle = getPedOccupiedVehicle ( localPlayer )
			if theVehicle then return end

	if getElementData(localPlayer, "apontar") == false then
	setElementData(localPlayer, "apontar", true)

	triggerServerEvent ( "onClientSyncVOZ", localPlayer )
	end
        else
	setElementData(localPlayer, "apontar", false)

	triggerServerEvent ( "onClientSyncVOZparar", localPlayer )
    end
end
)


bindKey("lalt", "both",
    function(key, state)
		if state == "down" then
			local theVehicle = getPedOccupiedVehicle ( localPlayer )
			if theVehicle then return end
			
	if getElementData(localPlayer, "apontar") == false then
	setElementData(localPlayer, "apontar", true)

	triggerServerEvent ( "onClientDesmaiarON", localPlayer )
	end
        else
	setElementData(localPlayer, "apontar", false)

	triggerServerEvent ( "onClientSyncVOZparar", localPlayer )
    end
end
)



function setSellerLookAt(player)
	if(isElement(player)) then
	setPedLookAt(player, 0.0, 0.0, 0.0, 3000, localPlayer)
	end
end



addEventHandler("onClientRender", root, function()

	if not getElementData(localPlayer, "loggedin") then return end

	if(getTickCount() - (lastLocalHeadUpdate or 0) > 100) then
		local task = getElementData(localPlayer,"TASK")
		if(not task or task == "") then
			local x, y, z, lx, ly, lz = getCameraMatrix()
			local px, py, pz = getPedBonePosition(localPlayer, 4)
			local lookAtX = px - (x-lx) * 20.0
			local lookAtY = py - (y-ly) * 20.0
			local lookAtZ = pz - (z-lz) * 20.0
			setPedLookAt(localPlayer, lookAtX, lookAtY, lookAtZ, 3000)
			lastLocalHeadUpdate = getTickCount()
		end
	end

		if(getTickCount() - (lastRemoteHeadUpdate or 0) > 500) then
			-- Update LookAt
			local x, y, z, lx, ly, lz = getCameraMatrix()
			
			local lookAtOffsetX = math.ceil((x-lx)*20.0)
			local lookAtOffsetY = math.ceil((y-ly)*20.0)
			local lookAtOffsetZ = math.ceil((z-lz)*20.0)
			
			setElementData(localPlayer, "lookAtOffsetX", lookAtOffsetX)
			setElementData(localPlayer, "lookAtOffsetY", lookAtOffsetY)
			setElementData(localPlayer, "lookAtOffsetZ", lookAtOffsetZ)


			local streamInPlayers = getElementsByType("player", root, true)
			for k, v in pairs(streamInPlayers) do
				if not getElementData(v, "loggedin") then return end
				if isElement(v) and v ~= getLocalPlayer() then
					local px, py, pz = getPedBonePosition(v, 4)
					local lookAtX = px - (getElementData(v, "lookAtOffsetX") or 0)
					local lookAtY = py - (getElementData(v, "lookAtOffsetY") or 0)
					local lookAtZ = pz - (getElementData(v, "lookAtOffsetZ") or 0)
					setPedAimTarget(v, lookAtX, lookAtY, lookAtZ)
					setPedLookAt(v, lookAtX, lookAtY, lookAtZ, 3000)
				end
			end
			lastRemoteHeadUpdate = getTickCount()
	end
end
)

addCommandHandler("sur",
	function()
		if not getElementData(localPlayer, "loggedin") then return end
	if getElementData(localPlayer, "VoiceSUR") == false then
	setElementData(localPlayer, "VoiceSUR", true)

	setElementData(localPlayer, "minDist", 0)
	setElementData(localPlayer, "maxDist",7)
	setElementData(localPlayer, "maxVol",7)
    else
	setElementData(localPlayer, "VoiceSUR", false)
	setElementData(localPlayer, "minDist", 5)
	setElementData(localPlayer, "maxDist",25)
	setElementData(localPlayer, "maxVol",4)
	
    end
end
)

local screenW, screenH = guiGetScreenSize()


windowSize = {1130, 750}

windowSize = {(windowSize[1] * screenW / 1280) , (windowSize[2] * screenH / 720)}


local windowStart = {screenW * 0.029, screenH * 0.95 - windowSize[2]}


addEventHandler("onClientRender", root,
	function()
		if not getElementData(localPlayer, "loggedin") then return end
		if getElementData(localPlayer, "VoiceSUR") == true then
  
		dxDrawText("*Modo: Sussurro", (windowSize[1] * 0.2695) - 1, (windowSize[2] * 1.1405) - 1, (windowSize[1] * 0.1347) - 1, (windowSize[2] * 0.7344) - 1, tocolor(0, 0, 0, 174), 1.00, "default-bold", "left", "center", false, false, false, true)
        dxDrawText("*Modo: Sussurro", (windowSize[1] * 0.2695) + 1, (windowSize[2] * 1.1405) - 1, (windowSize[1] * 0.1347) + 1, (windowSize[2] * 0.7344) - 1, tocolor(0, 0, 0, 174), 1.00, "default-bold", "left", "center", false, false, false, true)
        dxDrawText("*Modo: Sussurro", (windowSize[1] * 0.2695) - 1, (windowSize[2] * 1.1405) + 1, (windowSize[1] * 0.1347) - 1, (windowSize[2] * 0.7344) + 1, tocolor(0, 0, 0, 174), 1.00, "default-bold", "left", "center", false, false, false, true)
        dxDrawText("*Modo: Sussurro", (windowSize[1] * 0.2695) + 1, (windowSize[2] * 1.1405) + 1, (windowSize[1] * 0.1347) + 1, (windowSize[2] * 0.7344) + 1, tocolor(0, 0, 0, 174), 1.00, "default-bold", "left", "center", false, false, false, true)
        dxDrawText("*Modo: #00B3FFSussurro", windowSize[1] * 0.2695, windowSize[2] * 1.1405, windowSize[1] * 0.1347, windowSize[2] * 0.7344, tocolor(239, 239, 239, 254), 1.00, "default-bold", "left", "center", false, false, false, true)

		else

			dxDrawText("*Modo: Normal", (windowSize[1] * 0.2695) - 1, (windowSize[2] * 1.1405) - 1, (windowSize[1] * 0.1347) - 1, (windowSize[2] * 0.7344) - 1, tocolor(0, 0, 0, 174), 1.00, "default-bold", "left", "center", false, false, false, true)
			dxDrawText("*Modo: Normal", (windowSize[1] * 0.2695) + 1, (windowSize[2] * 1.1405) - 1, (windowSize[1] * 0.1347) + 1, (windowSize[2] * 0.7344) - 1, tocolor(0, 0, 0, 174), 1.00, "default-bold", "left", "center", false, false, false, true)
			dxDrawText("*Modo: Normal", (windowSize[1] * 0.2695) - 1, (windowSize[2] * 1.1405) + 1, (windowSize[1] * 0.1347) - 1, (windowSize[2] * 0.7344) + 1, tocolor(0, 0, 0, 174), 1.00, "default-bold", "left", "center", false, false, false, true)
			dxDrawText("*Modo: Normal", (windowSize[1] * 0.2695) + 1, (windowSize[2] * 1.1405) + 1, (windowSize[1] * 0.1347) + 1, (windowSize[2] * 0.7344) + 1, tocolor(0, 0, 0, 174), 1.00, "default-bold", "left", "center", false, false, false, true)
			dxDrawText("*Modo: #F7FF00Normal", windowSize[1] * 0.2695, windowSize[2] * 1.1405, windowSize[1] * 0.1347, windowSize[2] * 0.7344, tocolor(239, 239, 239, 254), 1.00, "default-bold", "left", "center", false, false, false, true)
	
    end
	end
)







local screenW, screenH = guiGetScreenSize()

addEventHandler("onClientRender", root,
	function()
		if not getElementData(localPlayer, "loggedin") then return end
		if getElementData(localPlayer, "VoiceCor") == true then
  
		dxDrawText("*Voz: Ativado", (windowSize[1] * 0.2695) - 1, (windowSize[2] * 1.1005) - 1, (windowSize[1] * 0.1347) - 1, (windowSize[2] * 0.7344) - 1, tocolor(0, 0, 0, 174), 1.00, "default-bold", "left", "center", false, false, false, true)
        dxDrawText("*Voz: Ativado", (windowSize[1] * 0.2695) + 1, (windowSize[2] * 1.1005) - 1, (windowSize[1] * 0.1347) + 1, (windowSize[2] * 0.7344) - 1, tocolor(0, 0, 0, 174), 1.00, "default-bold", "left", "center", false, false, false, true)
        dxDrawText("*Voz: Ativado", (windowSize[1] * 0.2695) - 1, (windowSize[2] * 1.1005) + 1, (windowSize[1] * 0.1347) - 1, (windowSize[2] * 0.7344) + 1, tocolor(0, 0, 0, 174), 1.00, "default-bold", "left", "center", false, false, false, true)
        dxDrawText("*Voz: Ativado", (windowSize[1] * 0.2695) + 1, (windowSize[2] * 1.1005) + 1, (windowSize[1] * 0.1347) + 1, (windowSize[2] * 0.7344) + 1, tocolor(0, 0, 0, 174), 1.00, "default-bold", "left", "center", false, false, false, true)
        dxDrawText("*Voz: #00FF00Ativado", windowSize[1] * 0.2695, windowSize[2] * 1.1005, windowSize[1] * 0.1347, windowSize[2] * 0.7344, tocolor(239, 239, 239, 254), 1.00, "default-bold", "left", "center", false, false, false, true)

		else

        dxDrawText("*Voz: Desativado", (windowSize[1] * 0.2695) - 1, (windowSize[2] * 1.1005) - 1, (windowSize[1] * 0.1347) - 1, (windowSize[2] * 0.7344) - 1, tocolor(0, 0, 0, 174), 1.00, "default-bold", "left", "center", false, false, false, true)
        dxDrawText("*Voz: Desativado", (windowSize[1] * 0.2695) + 1, (windowSize[2] * 1.1005) - 1, (windowSize[1] * 0.1347) + 1, (windowSize[2] * 0.7344) - 1, tocolor(0, 0, 0, 174), 1.00, "default-bold", "left", "center", false, false, false, true)
        dxDrawText("*Voz: Desativado", (windowSize[1] * 0.2695) - 1, (windowSize[2] * 1.1005) + 1, (windowSize[1] * 0.1347) - 1, (windowSize[2] * 0.7344) + 1, tocolor(0, 0, 0, 174), 1.00, "default-bold", "left", "center", false, false, false, true)
        dxDrawText("*Voz: Desativado", (windowSize[1] * 0.2695) + 1, (windowSize[2] * 1.1005) + 1, (windowSize[1] * 0.1347) + 1, (windowSize[2] * 0.7344) + 1, tocolor(0, 0, 0, 174), 1.00, "default-bold", "left", "center", false, false, false, true)
        dxDrawText("*Voz: #FF0000Desativado", windowSize[1] * 0.2695, windowSize[2] * 1.1005, windowSize[1] * 0.1347, windowSize[2] * 0.7344, tocolor(239, 239, 239, 254), 1.00, "default-bold", "left", "center", false, false, false, true)

    end
	end
)



addCommandHandler("shown", function()
	if getElementData(localPlayer, "acc:admin") > 0 or getElementData(localPlayer, "acc:guard") > 0 then
		settings.anames = not settings.anames
		
		if settings.anames then
			outputChatBox("#7cc576[BTCMTA - Nomes]: #ffffffVocê ativou com sucesso #ffffffos nomes.", 255, 255, 255, true)
			settings.maxdis = 200
		else
			outputChatBox("#7cc576[BTCMTA - Nomes]: #ffffffVocê desativou com sucesso #ffffffos nomes.", 255, 255, 255, true)
			settings.maxdis = 20
		end
	end
end)

--[[
local lastClick = getTickCount()

addEventHandler ("onClientRender",getRootElement(),
	function ()
		local cTick = getTickCount ()
		if cTick-lastClick >= 300000 then
			if getElementData(getLocalPlayer(),"afk") == false then
				local hp = getElementHealth (getLocalPlayer())
				if hp > 0 then
					setElementData (getLocalPlayer(),"afk",true)
					if isTimer (StartTimer) then 
						killTimer(StartTimer)
					end

					SimaTimer = setTimer(function ()
						StartTimer = setTimer(Timer,1000,0)
						if isTimer (SimaTimer) then 
							killTimer(SimaTimer)
						end
					end, 10*1000, 1 )
				end
			end
		end
	end
)

addEventHandler( "onClientRestore", getLocalPlayer(),
	function ()
		lastClick = getTickCount ()
		setElementData (getLocalPlayer(),"afk",false)
		sec = 0
		sech = setElementData(getLocalPlayer(),"Timer", 0)
		if isTimer (StartTimer) then 
			killTimer(StartTimer)
		end
	end
)

addEventHandler( "onClientMinimize", getRootElement(),
	function ()
		setElementData (getLocalPlayer(),"afk",true)
		sech = setElementData(getLocalPlayer(),"Timer", 0)
		sec = 0
		if isTimer (StartTimer) then 
			killTimer(StartTimer)
		end
		SimaTimer = setTimer(function ()
		end, 10*1000, 1 )
	end
)

addEventHandler( "onClientCursorMove", getRootElement( ),
    function ( x, y )
		lastClick = getTickCount ()
		if getElementData(getLocalPlayer(),"afk") == true then
			setElementData (getLocalPlayer(),"afk",false)
			sech = setElementData(getLocalPlayer(),"Timer", 0)
			if isTimer (StartTimer) then 
				killTimer(StartTimer)
			end
		end
    end
)


addEventHandler("onClientKey", getRootElement(), 
	function ()
		
		lastClick = getTickCount ()
		if getElementData(getLocalPlayer(),"afk") == true then

			setElementData (getLocalPlayer(),"afk",false)
			sech = setElementData(getLocalPlayer(),"Timer", 0)
			sec = 0
			if isTimer (StartTimer) then 
				killTimer(StartTimer)
			end
		end
	end
)
]]--

function checkForConsole()
	if isConsoleActive() then
		setElementData(getLocalPlayer(), "consoleTyping", true)	
	elseif not isConsoleActive() then
		setElementData(getLocalPlayer(), "consoleTyping", false)
	end
end
--setTimer ( checkForConsole, 100, 0 )

function checkForChat()
	if isChatBoxInputActive() then
		setElementData(getLocalPlayer(), "pmTyping", true)	
	elseif not isChatBoxInputActive() then
		setElementData(getLocalPlayer(), "pmTyping", false)
	end
end
--setTimer ( checkForChat, 100, 0 )
