...
function isMailCarrier(player)
	...
	if (exports.bgo_employment:getPlayerJob(true) == "Sedex") then return true end
	return false
end
...
function collectPackages(player, seat, vehicle)
	...
	local zone = "Las Venturas" --getZoneName(x,y,z, true)
	...
	exports.bgo_hud:dm("tem "..maxPackages.." pacotes no armazém que precisam ser transportados. Pegue-os e coloque-os em sua van.", 255, 200, 0)
...
function pickupPackage(player, dim)
	...
	pickups[source] = nil
	totalPickups = totalPickups - 1
	exports.bgo_hud:dm("Agora que você tem o pacote, coloque-o na parte de trás da sua van.", 255, 200, 0)
end
...
function createDestinations(player, mailvan, isLoaded)
	...
	local zone = "Las Venturas" --getZoneName(x1, y1, z1, true)
	...
	if (isLoaded) then
		exports.bgo_hud:dm("Todos os pacotes foram carregados. Seu primeiro pacote deve ser entregue a "..zone, 255, 200, 0)
		local maxPackages = vehIDs[getElementModel(mailvan)][2] or 5
		exports.bgo_hud:drawStat("MailCarrier", "Pacotes Carregados", (maxPackages).."/"..(maxPackages), 255, 200, 0)
	else
		exports.bgo_hud:dm("Entregar um pacote para o próximo local de entrega em "..zone, 255, 200, 0)
	end
...
function returnToWarehouse(player)
	...
	distance = getDistanceBetweenPoints3D(x1, y1, z1, x, y, z)
	exports.bgo_hud:dm("Todos os pacotes entregues! Retornar ao armazém para mais trabalho.", 255, 200, 0)
end
