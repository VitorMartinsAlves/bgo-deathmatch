...
{createPed ( 57, 2395.7707519531,1161.1572265625,10.8359375, 267.16064453125), 0, 0},
...
{createPed ( 144, 2096.7788085938,2241.6472167969,11.0234375, 176.31481933594), 0, 0}, --loja LV
...
{createPed ( 37, 1689.193, -1240.057, 14.984, 188.577 ), 0, 0 },
...
local teste1 = createColSphere(1316.5471191406,-896.48675537109,139.578125,3)
local teste2 = createColSphere(2396.9372558594,1161.2281494141,10.8359375,3)
...
local teste5 = createColSphere(2112.115, -1201.606, 123.866,3)
local teste6 = createColSphere(2096.7788085938,2241.6472167969,11.0234375,3)
...
local teste10 = createColSphere(1689.2092285156,-1240.0582275391,14.984375,3)
...
function detectAim( target )
    --local job = exports.bgo_employment:getPlayerJob(true)
    --if getPlayerTeam(localPlayer) == getTeamFromName("Policia") then return end
    
    if isElementWithinColShape(localPlayer, teste1) or isElementWithinColShape(localPlayer, teste2) or isElementWithinColShape(localPlayer, teste3) or isElementWithinColShape(localPlayer, teste4) or isElementWithinColShape(localPlayer, teste5) or isElementWithinColShape(localPlayer, teste6) or isElementWithinColShape(localPlayer, teste7) or isElementWithinColShape(localPlayer, teste8) or isElementWithinColShape(localPlayer, teste9) or isElementWithinColShape(localPlayer, teste10) then
    ...
    if ( target ) and ( getElementType( target ) == "ped" ) and (source == localPlayer) and getPedControlState("aim_weapon") and isItAPedToRob(target) then
        ...
        if (not robberyStarted) then
			...
			outputChatBox("#dc143c[AVISO]:#ffffff Não tem policia na cidade ( minimo 30 policiais ), vaza daqui!", 255, 255, 255, true)
            end
			end
        end
...
function payoutForSafe ( player )
	...
	if (getElementInterior(localPlayer) ~= 0) or (getElementDimension(localPlayer) ~= 0) then return end
        ...
        c = setTimer ( isRobberyFalseAgainisRobberyFalseAgainisRobberyFalseAgainisRobberyFalseAgain, 3600000, 1 )
...
local zone2 = createColCuboid(2358.36475, 1085.35803, 9.82031, 56.56787109375, 96.694702148438, 33.799932861328)
...
local zone6 = createColCuboid(2067.24414, 2224.27490, 7.38267, 34.775146484375, 49.1884765625, 20.199976348877) -- loja LV
local zone7 = createColCuboid(927.80475, -1370.13892, 12.08152, 11.592041015625, 20.926635742188, 4.3000005722046)
local zone8 = createColCuboid(1660.10242, -2301.18018, -2.18887, 46.776245117188, 29.052490234375, 9.4000004768372) --aeroporto
local zone9 = createColCuboid(647.63837, -1874.81714, 4.46094, 31.708984375, 15.641845703125, 4.900001907348)
local zone10 = createColCuboid(1672.77551, -1253.31885, 13.98438, 19.931030273438, 15.883178710938, 7.8999885559082)
...
addEventHandler ("onClientColShapeLeave", zone10, robberyCancelOnMarkerHit )
