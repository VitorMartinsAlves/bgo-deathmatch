...
function showOrHideHandbrakeGui (key,keystate)
	...
	if getElementData(getLocalPlayer(),"NGEvents:IsPlayerInEvent") then return end
...
addEventHandler ("onHandbrakeZoneChange", getRootElement(),
	function (zone)
		...
		if veh then
			...
			elseif zone == 3 then
				local handbrakeSound = playSound ("files/handbrake.wav")
		...
		if hand.show == true then
			...
			if veh then
				if getElementSpeed (veh) < 5 then
					dxDrawImage (hand.x,hand.y,64,256,"files/handbrakebg.png")
					dxDrawImage(hand.x,hand.currentY,64,16,"files/handbrakepointer.png")
				end
			else
				showHandbrakeGui (false)
			end
...
function informatePlayerAboutHandbrake ()
	...
	if playerVeh then
		if getVehicleEngineState (playerVeh) == true then
			outputChatBox ("Freio de mão puxado.", 255,168,0)
		end
	end
end
