...
local job_PedPos = {
	{227, 2471.218, 2382.843, 12.297, "Empregos", 176.86135864258},
	{227, 1103.4532470703,-1684.7548828125,13.538749694824	, "Empregos para LV", 0.69983232021332},
...
local jobs_Table = { 
	{"Pedreiro", "Pedreiro", 27, 0},
	{"Ifood", "ifood", 155, 0}, 
	{"Lixeiro", "Lixeiro", 309, 3},
	--{"Limpador", "Limpador", 16, 5}, 
	{"Sedex", "Sedex", 36, 4}, 
	{"Entregador de Gás", "Entregador de Gas", 184, 5}, 
	{"Motorista", "Motorista", 255, 15},  
	{"Motorista Rodoviário", "MotoristaR", 255, 17}, 
	{"Maquinista", "Maquinista", 153, 22},
	
	--{"Eletricista", "Eletricista", 177, 22}, 	
}
			...
			if (index > nextPage and elem < maxElem) then
				...
				if dobozbaVan(sx*210, sy*250-95+elem*(sy*50), sx*120, sy*40, x, y) then 
					...
					elseif jobs_Table[index][2] ~= localPlayer:getData("job") or localPlayer:getData("job") == "SemEmprego" or localPlayer:getData("job") == "0" or localPlayer:getData("job") == "" then 
						triggerServerEvent("trabalho", resourceRoot, localPlayer, jobs_Table[index][2],tonumber(jobs_Table[index][3]))
