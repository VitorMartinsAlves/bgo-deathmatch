...
local sX,sY = (screenW2/1360), (screenH2/768)
hudEnabled = nil	-- Is the Hud Enabled?


local SAFEZONE_X2 = sX*0.045
local SAFEZONE_Y2 = sY*0.65

local SAFEZONE_X3 = sX*0.01
local SAFEZONE_Y3 = sY*0.08
...
addEventHandler("onClientPreRender", root,
	...
	if not getElementData (localPlayer, "loggedin") then return end
		if getResourceFromName( "bgo_logo" ) and getResourceState ( getResourceFromName( "bgo_logo" ) ) == "running" then
			local logo = exports.bgo_logo:logo()
			if logo then
	        dxDrawImage(x*1248, y*12, x*90, y*90, logo, 0, 0, 0, tocolor(255, 254, 254, 255), false)
			end
		end
	end
)
...
function renderFPSandPing()
	...
	--local dX,dY,dW,dH = sX,sY*364,sX,sY*20
	...
	--dxDrawText(""..game_time.." | "..monthday.."/"..meses.."/20"..ano.." Ping: "..ping.." | FPS: "..fps, dX+1,dY,dW+1,dH, tocolor(0,0,0,255), 1, "clear", "right", "top")
	--dxDrawText(""..game_time.." | "..monthday.."/"..meses.."/20"..ano.." Ping: "..ping.." | FPS: "..fps, dX,dY+1,dW,dH+1, tocolor(0,0,0,255), 1, "clear", "right", "top")
	
	
	dxDrawText("Seu id: "..id.." | "..game_time.." | "..monthday.."/"..meses.."/20"..ano.." PING: "..ping.." | FPS: "..fps, sX*1948, sY*756, sX*1190, sY*90, tocolor(255,255,255,255/2), sY/1.5, "default", "right", "top")
...
function renderMoney()
	...
	local dX,dY,dW,dH = sX*1948, sY*756, sX*1190, sY*90 --sX-6,35+moneyY+wantedOff,sX-6,30
	--local dX,dY,dW,dH = dX-SAFEZONE_X, dY+SAFEZONE_Y, dW-SAFEZONE_X, dH+SAFEZONE_Y
...
addEventHandler("onClientElementDataChange", root, function(dataName, oldValue)
	if source == localPlayer and dataName == "char:money" and getElementData(localPlayer, "loggedin") then
        ...
        if newValue then
			...
			local som = playSound("money.mp3", false)
			setSoundVolume(som,0.03)
        end
...
function renderPlusMinusMoney()
    ...
    local dX,dY,dW,dH = sX*1998, sY*126, sX*1310, sY*90 --sX-6,70+wantedOff,sX-6,30
   -- local dX,dY,dW,dH = dX-SAFEZONE_X3, dY+SAFEZONE_Y3, dW-SAFEZONE_X3, dH+SAFEZONE_Y3
    dxDrawText(pm.."R$"..cash, dX+2,dY,dW+2,dH, tocolor(0,0,0,monA), 1.25, "pricedown", "right")
    dxDrawText(pm.."R$"..cash, dX,dY+2,dW,dH+2, tocolor(0,0,0,monA), 1.25, "pricedown", "right")
    dxDrawText(pm.."R$"..cash, dX-2,dY,dW-2,dH, tocolor(0,0,0,monA), 1.25, "pricedown", "right")
    dxDrawText(pm.."R$"..cash, dX,dY-2,dW,dH-2, tocolor(0,0,0,monA), 1.25, "pricedown", "right")
    dxDrawText(pm.."R$"..cash, dX,dY,dW,dH, tocolor(mr,mg,mb,monA), 1.25, "pricedown", "right")
end
...
function renderWeapons()
	...
	if (weapon >= 15 and weapon ~= 40 and weapon <= 44 or weapon >= 46) then
		...
		--dX,dY,dW,dH = sX*1148, sY*706, sX*545, sY*90 --sX-6,35+wantedOff,sX-6,30
		--local dX,dY,dW,dH = dX-SAFEZONE_X, dY+SAFEZONE_Y, dW-SAFEZONE_X, dH+SAFEZONE_Y
		
		
		--dxDrawText(clip, dX+2,dY,dW+2,dH, tocolor(0,0,0,255), sX/1.6, "pricedown", "right")
		--dxDrawText(clip, dX,dY+2,dW,dH+2, tocolor(0,0,0,255), sX/1.6, "pricedown", "right")
		--dxDrawText(clip, dX-2,dY,dW-2,dH, tocolor(0,0,0,255), sX/1.6, "pricedown", "right")
		--dxDrawText(clip, dX,dY-2,dW,dH-2, tocolor(0,0,0,255), sX/1.6, "pricedown", "right")
		--dxDrawText(clip, dX,dY,dW,dH, tocolor(110,110,110,255), sX/1.6, "pricedown", "right")
			-- Total Ammo
		--local dX,dY,dW,dH = sX-6-xoff,35+wantedOff,sX-6-xoff,30

		--local dX,dY,dW,dH = dX-SAFEZONE_X, dY+SAFEZONE_Y, dW-SAFEZONE_X, dH+SAFEZONE_Y
		dxDrawText("#cccccc"..clip.."\n#ffffff"..ammo-clip.."", sX*1070, sY*1480, sX/2, 0, tocolor(0,0,0,255), sY/1.5, "pricedown","center", "center",false,false,false,true)
		--dxDrawText(ammo-clip, dX,dY+2,dW,dH+2, tocolor(0,0,0,255), sX/1.6, "pricedown", "right")
		--dxDrawText(ammo-clip, dX-2,dY,dW-2,dH, tocolor(0,0,0,255), sX/1.6, "pricedown", "right")
		--dxDrawText(ammo-clip, dX,dY-2,dW,dH-2, tocolor(0,0,0,255), sX/1.6, "pricedown", "right")
		--dxDrawText(ammo-clip, dX,dY,dW,dH, tocolor(220,220,220,255), sX/1.6, "pricedown", "right")
	else
		xoff = 0
		weapLen = 0
	end
	...
	local dX,dY,dW,dH = sX*415, sY*726, sX*90, sY*30  --sX-133-weapLen,35+wantedOff,128,40
