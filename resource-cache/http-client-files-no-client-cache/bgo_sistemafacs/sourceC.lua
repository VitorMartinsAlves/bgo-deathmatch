...
addEventHandler("onClientClick", root, function(button, state, absX, absY, elementX, elementY, elementZ, element)
	if button == "right" and state == "down" and element and getElementType(element)=="player" then 
		...
		if exports.bgo_admin:isPlayerFaccao(localPlayer) or getElementData(localPlayer,"acc:id") == 1 and element ~= localPlayer then
		if not exports.bgo_admin:isPlayerFaccao(element) then
		panelX = sx*970
		panelY = sy*250
		Player = element	
		show = true
		...
		triggerEvent("bgo>info", localPlayer,"Revistando", "Não pode revistar pessoas de facçoes!", "info")
		end
		end
		end
	elseif button == "left" and state == "down" and show then 
		for index, value in ipairs (active_Menu) do 
			if dobozbaVan(panelX+10, panelY-20+index*55, panelSize[1]-20, 50, absX, absY) then 
				if value[1] == "Revistar" then  
					triggerServerEvent("vPhone", localPlayer, localPlayer, Player, getElementData(Player, "char:money"))
					triggerServerEvent("btcMTA->#setPlayerMe", localPlayer, localPlayer, "#ff00ffRevistando a pessoa!")
...
function createPlayerPanel()
	...
	dxDrawText("Painel Revistar", panelX+panelSize[1]/2, panelY+25/2, panelX+panelSize[1]/2, panelY+25/2, tocolor(255, 255, 255, 230), 2, "default", "center", "center", false, false, false, true)
