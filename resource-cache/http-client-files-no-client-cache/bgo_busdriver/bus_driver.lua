...
-- Date: 27 Jul 2014
-- Resource: busdriver/bus_driver.lua
-- Version: 1.0
----------------------------------------->>
...
function isBusDriver(player)
	...
	if (exports.bgo_employment:getPlayerJob(true) ~= "Motorista") then return false end
...
function startMission(player, seat)
	...
	exports.bgo_hud:dm("BUS DRIVER: Faça o seu caminho para o seu primeiro destino em "..getZoneName(x, y, z), 255, 200, 0)
	exports.bgo_hud:drawStat("BusDriver", "Passageiros", "0", 255, 200, 0)
end
...
function loadPassangers(player, dim)
	...
	if (exports.bgo_util:getElementSpeed(getPedOccupiedVehicle(player)) > 30) then
		exports.bgo_hud:dm("BUS DRIVER: Você deve estar indo mais devagar do que 30 mph para que os passageiros embarquem no seu ônibus.", 255, 125, 0)
		return
	end
	...
	if pagar > 4 then
	triggerServerEvent("busdriver.completeMission", resourceRoot)
	...
	local delBlip = createBlip( x, y, z, 57, 0, 0, 0, 255 )
		exports.bgo_hud:dm("BUS DRIVER: Sua próxima parada é em "..getZoneName(x, y, z)..", vá até lá.", 255, 200, 0)
...
function terminateJob(job)
	if (job ~= "Motorista") then return end
...
function renderLoadPassengers()
	...
	exports.bgo_hud:drawStat("BusDriver", "Passageiros", r_passengers, 255, 200, 0)
end
