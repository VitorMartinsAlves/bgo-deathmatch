...
addEventHandler("onClientClick", root, function(button, state, absX, absY, elementX, elementY, elementZ, element)
	if button == "right" and state == "down" and element and getElementType(element)=="vehicle" then 
		...
		if getElementModel(element) == 416 or getElementModel(element) == 488 then 
	...
	elseif button == "left" and state == "down" and show then 
		for index, value in ipairs (active_Menu) do 
			if dobozbaVan(panelX+10, panelY-20+index*55, panelSize[1]-20, 50, absX, absY) then 
				if value[1] == "Pegar/Guardar Maca" then  
					triggerServerEvent("GiveMaca", localPlayer, localPlayer, Player)


					elseif value[1] == "Tirar cidadao do veiculo" then 
					triggerServerEvent("TirarNoVeiculo", localPlayer, localPlayer, Player)
...
function createPlayerPanel()
	...
	dxDrawText("Painel Medico", panelX+panelSize[1]/2, panelY+25/2, panelX+panelSize[1]/2, panelY+25/2, tocolor(255, 255, 255, 230), 2, "default", "center", "center", false, false, false, true)
