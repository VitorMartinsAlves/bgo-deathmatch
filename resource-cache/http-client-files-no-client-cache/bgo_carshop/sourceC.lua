...
local font2 = "default" --dxCreateFont("myriadproregular.ttf",10) --<[ Font ]>--
local text = "bgoMTA - #00aeefConcessionária de carros da cidade"
local camPos = {}
camPos[1] = {} -- Campos[érték] táblázata
camPos[1]["start"] = {1461.5465087891,-826.03656005859,182.8125, 212.73910522461, 212.73910522461, 212.73910522461}


--camPos[1]["start"] = {1671.517, -1270.301, 16.018, 1663.557, -1267.408, 14.813}
...
camPos[2]["start"] = {2115.9931640625, 1921.560546875, 12.436599731445, 2116.76171875, 1921.1206054688, 11.972104072571}
...
camPos[3]["start"] = {2124.7902832031, 1907.1014404297, 12.116200447083, 2124.4919433594, 1908.0008544922, 11.796879768372}
...
camPos[4]["start"] = {2128.9689941406, 1913.9185791016, 12.037300109863, 2128.03515625, 1914.1320800781, 11.750021934509}
...
camPos[5]["start"] = {2118.3969726563, 1926.8110351563, 15.134300231934, 2118.6647949219, 1925.9877929688, 14.633818626404}
...
local carposX, carposY, carposZ = 1454.365, -816.774, 181.813 --1662.745, -1264.965, 14.813 
...
local pedPos = {
	{187, 1661.917, -1257.191, 14.813, "Vendedor Elias", 176.22927856445},
	

	--[[
	{101, 2200.8740234375, 1394.7998046875, 11.0625, "Kilian", 180},
	{15, 1948.5986328125, 2068.6491699219, 11.060997009277, "Ozzy", 90},
	{134, 543.19769287109, -1293.8350830078, 17.2421875, "Tobi", 0},]]--
}
local create = false
...
function pedLetrehozas() 
	for index,value in ipairs (pedPos) do
		...
		setElementData(blip,"blipName", "Loja de Veiculos")


		ped[index]:setData("carshop-->Ped", true)
...
function pedKill()
	if getElementData(source, "carshop-->Ped") then
		cancelEvent()
	end
end
...
function onPedKatt(button, state, absoluteX, absoluteY, worldX, worldY, worldZ, clickedElement)
	if clickedElement and button == "right" and state == "down" and getElementData(clickedElement, "carshop-->Ped") and getElementType(clickedElement) == "ped" and not create then
	...
	elseif clickedElement and button == "right" and state == "down" and getElementData(clickedElement, "carshop-->Ped") and getElementType(clickedElement) == "ped" and create then
	...
	if create and button == "left" and state == "down" and panelType == "CarShop" then 
		for index, value in ipairs (color) do 
			if dobozbaVan(Size2+15+index*35, panelPosY+298, 32, 32, absoluteX, absoluteY) then 
	...
	elseif create and button == "left" and state == "down" and panelType == "Vasarlas" then 
		if dobozbaVan(kepernyom[1]/2-300/2+15, kepernyom[2]/2-200/2+160, 100, 25, absoluteX, absoluteY) then 
			...
			elseif getVehicleLimits(sellVehicle[Jelenkocsi][1]) < sellVehicle[Jelenkocsi][5] or sellVehicle[Jelenkocsi][5] == 0 then 
				...
				triggerServerEvent("buyVehicleSever", localPlayer, localPlayer, sellVehicle[Jelenkocsi][1], color[selectedColor][1], color[selectedColor][2], color[selectedColor][3], amount, Jelenkocsi, number)
				panelType = "CarShop"
			end
		elseif dobozbaVan(kepernyom[1]/2-300/2+300-115, kepernyom[2]/2-200/2+160, 100, 25, absoluteX, absoluteY) then
			...
			local number = element:getData("carshop:type")
			triggerServerEvent("buyVehiclePPSever", localPlayer, localPlayer, sellVehicle[Jelenkocsi][1], color[selectedColor][1], color[selectedColor][2], color[selectedColor][3] , pp, Jelenkocsi, number)
			panelType = "CarShop"
		end
	end
end
...
addEvent("returnVasarlas",true)
addEventHandler("returnVasarlas",root,function(vehID)
...
function createVehicleFunction ()
	...
	setElementRotation(car, 0, 0, 149.991)
	setElementCollisionsEnabled(car, false)
	--if isTimer(timer) then
	--killTimer(timer)
	--end
	--timer = setTimer(function() local x, y, z = getElementRotation(car) setElementRotation(car, x, y, z+3) end, 100, 0)
end
...
function createCarshopPanel ()
	if panelType == "CarShop" then 
		...
		-500, 0 ,0,
		0, 0, 0,
		(getTickCount()-Tick)/1000, "OutQuad")
		
		dxDrawImage(x, y,sx*900,sy*800, "files/bg.png", 0, 0, 0, tocolor(255,255,255,255))
		...
		dxDrawText("Pressione\n\n'backspace' para fechar\n\n'enter' para comprar!\n\nSeta < e > para mudar o veiculo!",Size2+50, panelPosY-100+panelSize[2], panelSize[1], panelSize[2]-425, tocolor(0, 0, 0, 255),sx/1, font2, "center", "top", false, false, false, true) --<[  Felirat kiírás ]>--
		dxDrawText("Pressione\n\n#D24D57'backspace' #ffffffpara fechar\n\n#87D37C'enter'#ffffff para comprar!\n\nSeta #87D37C< #ffffffe #87D37C> #ffffffpara mudar o veiculo!",Size2+51, panelPosY-101+panelSize[2], panelSize[1], panelSize[2]-425, tocolor(255, 255, 255, 255),sx/1, font2, "center", "top", false, false, false, true) --<[  Felirat kiírás ]>--
		...
		dxDrawText(sellVehicle[Jelenkocsi][2], Size2+51, panelPosY+141, panelSize[1], panelSize[2]-425, tocolor(0, 0, 0,255), sx/0.8, font2, "center","top", false, false, false,true) --<[  Felirat kiírás ]>--
		dxDrawText(sellVehicle[Jelenkocsi][2], Size2+50, panelPosY+140, panelSize[1], panelSize[2]-425, tocolor(255, 255, 255,255), sx/0.8, font2, "center","top", false, false, false,true) --<[  Felirat kiírás ]>--


		dxDrawText("Preço: R$: "..penz_darabolas(sellVehicle[Jelenkocsi][3]) .. "\n Dinheiro Vip: "..penz_darabolas(sellVehicle[Jelenkocsi][4]).. "", Size2+51, panelPosY+221, panelSize[1], panelSize[2]-425, tocolor(0, 0, 0,255), sx/1.0, font2, "center","top", false, false, false,true) --<[  Felirat kiírás ]>--

		dxDrawText("Preço: #00aeefR$: "..penz_darabolas(sellVehicle[Jelenkocsi][3]) .. "\n#ffffff Dinheiro Vip: #00aeef"..penz_darabolas(sellVehicle[Jelenkocsi][4]).. "", Size2+50, panelPosY+220, panelSize[1], panelSize[2]-425, tocolor(255, 255, 255,255), sx/1.0, font2, "center","top", false, false, false,true) --<[  Felirat kiírás ]>--
		...
		if sellVehicle[Jelenkocsi][5] > 0 then 
			...
			dxDrawText("Limite de compras: ".. color .. getVehicleLimits(sellVehicle[Jelenkocsi][1]).. " / "..sellVehicle[Jelenkocsi][5], Size2+51, panelPosY+181, panelSize[1], panelSize[2]-425, tocolor(0, 0, 0,255), sx/1.0, font2, "center","top", false, false, false,true) --<[  Felirat kiírás ]>--
			dxDrawText("Limite de compras: ".. color .. getVehicleLimits(sellVehicle[Jelenkocsi][1]).. " #ffffff/ #00aeef"..sellVehicle[Jelenkocsi][5], Size2+50, panelPosY+180, panelSize[1], panelSize[2]-425, tocolor(255, 255, 255,255), sx/1.0, font2, "center","top", false, false, false,true) --<[  Felirat kiírás ]>--

		else
			dxDrawText("Não há #F5D76Elimite #ffffffpara este veículo", Size2+50, panelPosY+361, panelSize[1], panelSize[2]-426, tocolor(0, 0, 0,255), sx/1.0, font2, "center","top", false, false, false,true) --<[  Felirat kiírás ]>--
			dxDrawText("Não há #F5D76Elimite #ffffffpara este veículo", Size2+50, panelPosY+360, panelSize[1], panelSize[2]-425, tocolor(255, 255, 255,255), sx/1.0, font2, "center","top", false, false, false,true) --<[  Felirat kiírás ]>--
		...
		for index, value in ipairs (color) do 
			if isInSlot(Size2+15+index*35, panelPosY+298, 32, 32) then 
				dxDrawRectangle(Size2+15+index*35, panelPosY+298, 32, 32, tocolor(value[1], value[2], value[3]))
			else
				dxDrawRectangle(Size2+15+index*35, panelPosY+300, 30, 30, tocolor(value[1], value[2], value[3]))
			end
		end
...
function updateCamPosition ()
		...
		local _, _, z2 = interpolateBetween ( 
		0, 0 ,z+0.4,
		0, 0, z+0.4,
		(getTickCount()-Tick)/200, "SineCurve")
		setElementRotation(car, x, y, z2)

--[[
		setElementRotation(car, 0, 0, 10)
