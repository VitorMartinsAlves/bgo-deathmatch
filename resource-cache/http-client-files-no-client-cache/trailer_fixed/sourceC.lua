	...
	function (vehicle)
		if havePermission.impoundTow then
			if getElementModel(vehicle) == TRUCK_MODEL then
				...
				cableHolder[vehicle] = getElementData(vehicle, "cableHolder")

				rampState[vehicle] = getElementData(vehicle, "packerState") or "up"
				rampInAnim[vehicle] = false
...
addEventHandler( "onClientElementDestroy", getRootElement(),
	function ()
		...
		if getElementModel(source) == TRUCK_MODEL then
			...
			setElementData(source, "cableAttachment", nil)
			...
			for i = 1, #attached do
				...
				setElementData(attached[i], "cableAttachment", nil)
				setElementData(attached[i], "carAnimation", false)
				setElementData(attached[i], "carPlacedOnTruck", false)
				carInAnim[attached[i]] = nil
				cableAttachment[attached[i]] = nil
				triggerServerEvent("removergrude", localPlayer, attached[i])
	...
	if source and getElementType(source) == 'vehicle' then 
		if tostring(dataName) == 'carPlacedOnTruck' then 
			if not getElementData(source, dataName) then 
					...
					--setElementData(v.vehicle, "cableAttachment", nil)
					setElementData(v.vehicle, "carAnimation", false)
					--setElementData(v.vehicle, "carPlacedOnTruck", false)
					--cableAttachment[v.vehicle] = nil
					carInAnim[v.vehicle] = nil
					end
				end	
			end
		end
...
addEventHandler("onClientElementDataChange", getRootElement(),
	function (dataName, oldVal, newValue)
		if dataName == "cableAttachment" then
			cableAttachment[source] = getElementData(source, "cableAttachment") or {}
		end
		...
		if dataName == "packerState" then
			...
			if rampState[source] == "down" then
				...
				table.insert(effect.parts, {
					...
					endPos = {cx, (cy or 0) - 3.7, (cz or 0) - 0.135},
					endRot = {20, 0, 0},
					sound = "rotdown"
				})
			end
		...
		if dataName == "carAnimation" then
			...
			if dataVal then
				...
				local targetPosX, targetPosY, targetPosZ = getPositionFromElementOffset(carAnimation[source][1], 0, (rampY or 0) - 1.75, rampZ)
	...
	function ()
		...
		for k, v in pairs(rampAnimation) do
			if v.parts and v.current then
				...
				if isElement(v.vehicle) and dat then
					local elapsedTime = currentTime - v.startTime
					local progress = elapsedTime / dat.duration
					...
					if isElementStreamedIn(holder) then
						if isElementOnScreen(veh) or isElementOnScreen(holder) then
							...
							if dist <= 100 then
								...
								if holdertype == "vehicle" and getElementDimension(holder) == 0 then
		...
		if isElement(myTowTruck) then
			...
			if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) > 20 then
				--outputChatBox("Ilyen messzire nem ér el a kábel!", 255, 0, 0, true)
				...
				if state ~= "up" then
					...
					for k, v in pairs(keys) do
						if v == key then
							...
							if press then
								--outputChatBox("Előbb húzd be a rámpát!", 255, 255, 255, true)
							end
						end
					end
				end
						...
						if not rampInAnim[veh] then
							...
							elseif key == "cardisc" then
								if getElementData(veh, "packerState") == "down" then
									...
									--outputChatBox("Előszőr engedd ki a rámpát!", 255, 255, 255, true)
								end
					...
					elseif myTowTruck then
						if isElement(clickedElement) then
							if clickedElement ~= myTowTruck then
								...
								if getElementDimension(clickedElement) == 0 then
									--if getVehicleType(clickedElement) == "Automobile" then
										...
										if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) <= 5 then
											...
											if isInsideColShape(detectorColShape, x, y, z) and dist <= 2.5 then
												...
												--outputChatBox("Az autó nincs az autószállító mögött!", 255, 255, 255, true)
											end
										else
											--outputChatBox("Túl messze vagy a járműtől!", 255, 255, 255, true)
										end
									--else
										--outputChatBox("Csak autókat foglalhatsz le.", 255, 255, 255, true)
									end
								end
							end
						end
...
addEventHandler("onClientRender", getRootElement(),
	function ()
		...
		if havePermission.impoundTow then
			...
			if getElementModel(theTruck) == TRUCK_MODEL then
				...
				if isElement(theTruck) and getVehicleSpeed(theTruck) < 40 then
					...
					for i = 1, #vehicles do
						...
						if isElement(veh) and getElementDimension(veh) == 0 then
						...
						if isElementOnScreen(veh) then
							...
							if dist <= 2.35 then
								...
								if x and y then
									...
									if isElement(holder) and getElementType(holder) == "vehicle" and getElementDimension(holder) == 0 then
										local pictureColor = tocolor(255, 255, 255)
										local theCarStatus = getElementData(holder, "carAnimation") or {}
